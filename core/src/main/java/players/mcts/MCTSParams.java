...
public class MCTSParams extends PlayerParameters {
    ...
    public MCTSEnums.Information information = MCTSEnums.Information.Information_Set;  // this should be the default in TAG, given that most games have hidden information
    public MCTSEnums.MASTType MAST = MCTSEnums.MASTType.None;
    ...
    public MCTSEnums.SelectionPolicy selectionPolicy = MCTSEnums.SelectionPolicy.SIMPLE;  // In general better than ROBUST
    public MCTSEnums.TreePolicy treePolicy = MCTSEnums.TreePolicy.UCB;
    public MCTSEnums.OpponentTreePolicy opponentTreePolicy = MCTSEnums.OpponentTreePolicy.OneTree;
    public boolean paranoid = false;
    public MCTSEnums.Strategies rolloutType = MCTSEnums.Strategies.RANDOM;
    ...
    public MCTSEnums.RolloutTermination rolloutTermination = MCTSEnums.RolloutTermination.DEFAULT;
    ...
    public MCTSParams() {
        ...
        addTunableParameter("rolloutType", MCTSEnums.Strategies.RANDOM, Arrays.asList(MCTSEnums.Strategies.values()));
        addTunableParameter("oppModelType", MCTSEnums.Strategies.RANDOM, Arrays.asList(MCTSEnums.Strategies.values()));
        ...
        addTunableParameter("rolloutTermination", MCTSEnums.RolloutTermination.DEFAULT, Arrays.asList(MCTSEnums.RolloutTermination.values()));
        addTunableParameter("opponentModelParams", ITunableParameters.class);
        addTunableParameter("opponentModel", new RandomPlayer());
        addTunableParameter("information", MCTSEnums.Information.Information_Set, Arrays.asList(MCTSEnums.Information.values()));
        addTunableParameter("selectionPolicy", MCTSEnums.SelectionPolicy.SIMPLE, Arrays.asList(MCTSEnums.SelectionPolicy.values()));
        addTunableParameter("treePolicy", MCTSEnums.TreePolicy.UCB, Arrays.asList(MCTSEnums.TreePolicy.values()));
        addTunableParameter("opponentTreePolicy", MCTSEnums.OpponentTreePolicy.OneTree, Arrays.asList(MCTSEnums.OpponentTreePolicy.values()));
        addTunableParameter("exploreEpsilon", 0.1);
        addTunableParameter("heuristic", (IStateHeuristic) AbstractGameState::getHeuristicScore);
        addTunableParameter("MAST", MCTSEnums.MASTType.None, Arrays.asList(MCTSEnums.MASTType.values()));
    ...
    public void _reset() {
        ...
        if (information == MCTSEnums.Information.Closed_Loop)
        ...
        useMAST = MAST != MCTSEnums.MASTType.None;
    }
    ...
    public AbstractPlayer getOpponentModel() {
        if (opponentModel == null) {
            if (oppModelType == MCTSEnums.Strategies.PARAMS)
    ...
    public AbstractPlayer getRolloutStrategy() {
        if (rolloutPolicy == null) {
            if (rolloutType == MCTSEnums.Strategies.PARAMS)
