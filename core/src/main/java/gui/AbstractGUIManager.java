...
public abstract class AbstractGUIManager {
    ...
    protected void updateActionButtons(AbstractPlayer player, AbstractGameState gameState) {
        if (gameState.getGameStatus() == CoreConstants.GameResult.GAME_ONGOING && !(actionButtons == null)) {
            List<core.actions.AbstractAction> actions = player.getForwardModel().computeAvailableActions(gameState, gameState.getCoreGameParameters().actionSpace);
        ...
        if (!perspectiveSet.isEmpty()) {
            ...
            game.addListener(new IGameListener() {
                ...
                public void onEvent(evaluation.metrics.Event event) {
                    if (event.type == evaluation.metrics.Event.GameEvent.ACTION_CHOSEN) {
                        history.add("Player " + event.state.getCurrentPlayer() + " : " + event.action.getString(game.getGameState(), perspectiveSet));
                    } else if (event.type == evaluation.metrics.Event.GameEvent.GAME_EVENT) {
    ...
    protected static class ActionButton extends JButton {
        core.actions.AbstractAction action;
        ...
        public void setButtonAction(core.actions.AbstractAction action, AbstractGameState gameState) {
            this.action = action;
            if (action != null) setText(action.getString(gameState));
            else setText("");
        }

        public void setButtonAction(core.actions.AbstractAction action, String actionText) {
            this.action = action;
            setText(actionText);
        }
