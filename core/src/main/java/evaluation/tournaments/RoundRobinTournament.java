...
public class RoundRobinTournament extends AbstractTournament {
    ...
    public RoundRobinTournament(List<? extends AbstractPlayer> agents, GameType gameToPlay, int playersPerGame,
        ...
        this.tournamentMode = switch (config.get(RunArg.mode).toString().toUpperCase()) {
            case "EXHAUSTIVE" -> TournamentMode.EXHAUSTIVE;
            case "EXHAUSTIVESP" -> TournamentMode.EXHAUSTIVE_SELF_PLAY;
            case "ONEVSALL" -> TournamentMode.ONE_VS_ALL;
            default -> TournamentMode.RANDOM;
        };
        if (tournamentMode == TournamentMode.EXHAUSTIVE && nTeams > this.agents.size()) {
        ...
        switch (tournamentMode) {
            ...
            case EXHAUSTIVE:
                boolean selfPlay = tournamentMode == TournamentMode.EXHAUSTIVE_SELF_PLAY;
            ...
            case EXHAUSTIVE_SELF_PLAY:
                ...
                if (matchUp.size() == nTeams) {
                    ...
                    for (Integer agentID : this.allAgentIds) {
                        if (tournamentMode == TournamentMode.EXHAUSTIVE_SELF_PLAY || !matchUp.contains(agentID)) {
                            matchUp.add(agentID);
                            createAndRunMatchUp(matchUp);
    ...
    protected void evaluateMatchUp(List<Integer> agentIDsInThisGame, int nGames, List<Integer> seeds) {
        ...
        for (int agentID : agentIDsInThisGame)
            matchUpPlayers.add(tournamentMode == TournamentMode.EXHAUSTIVE_SELF_PLAY ? this.agents.get(agentID).copy() : this.agents.get(agentID));
