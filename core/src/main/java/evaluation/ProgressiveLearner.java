...
public class ProgressiveLearner {
    ...
    public ProgressiveLearner(String[] args) {
        ...
        try {
            gameToPlay = GameType.valueOf(Utils.getArg(args, "game", ""));
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException("A Game must be specified");
        }

        nPlayers = Utils.getArg(args, "nPlayers", 2);
        matchups = Utils.getArg(args, "matchups", 1);
        finalMatchups = Utils.getArg(args, "finalMatchups", 1000);
        iterations = Utils.getArg(args, "iterations", 100);
        maxExplore = Utils.getArg(args, "explore", 0.0);
        verbose = Utils.getArg(args, "verbose", false);
        elite = Utils.getArg(args, "elite", iterations + 1);
        agentsPerGeneration = new AbstractPlayer[iterations];
        dataFilesByIteration = new String[iterations];
        String learnerDefinition = Utils.getArg(args, "learner", "");
        if (learnerDefinition.equals(""))
            throw new IllegalArgumentException("Must specify a learner file");
        learner = JSONUtils.loadClassFromFile(learnerDefinition);
        String listenerString = Utils.getArg(args, "listener", "");
        if (listenerString.equals(""))
            throw new IllegalArgumentException("Must specify a listener file");
        featureListener = JSONUtils.loadClassFromFile(listenerString);
        prefix = Utils.getArg(args, "prefix", "ProgLearn");

        learnedFilesByIteration = new String[iterations];
        player = Utils.getArg(args, "player", "");
        String gameParams = Utils.getArg(args, "gameParams", "");
        dataDir = Utils.getArg(args, "dir", "");

        params = AbstractParameters.createFromFile(gameToPlay, gameParams);

        heuristic = Utils.getArg(args, "heuristic", "");
    }
