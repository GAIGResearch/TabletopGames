        ...
        if (largestArmyOwner == -1) {
            ...
            for (int i = 0; i < knights.length; i++) {
                if (knights[i] > max && largestArmyOwner != i) {
                    logEvent(CatanMetrics.CatanEvent.LargestArmySteal, String.valueOf(i));
...
//            }
    ...
    public Building[] getBuildings(CatanTile tile) {
        Building[] buildings = new Building[CatanConstants.HEX_SIDES];
    ...
    public Edge[] getRoads(CatanTile tile) {
        Edge[] roads = new Edge[CatanConstants.HEX_SIDES];
        for (int i = 0; i < CatanConstants.HEX_SIDES; i++) {
