...
public class CatanForwardModel extends StandardForwardModel {
    ...
    private GraphBoardWithEdges extractGraphFromBoard(CatanTile[][] board, CatanParameters cp, Random rnd) {
        ...
        for (CatanTile[] catanTiles : board) {
            for (CatanTile tile : catanTiles) {
                for (int vertex = 0; vertex < CatanConstants.HEX_SIDES; vertex++) {
                    ...
                    if (tile.getVerticesBoardNodeIDs()[vertex] == -1) {
                        ...
                        if (Arrays.stream(neighbourCoords[0]).max().getAsInt() < board.length &&
                            ...
                            board[neighbourCoords[0][0]][neighbourCoords[0][1]].setVertexBoardNodeID((vertex + 2) % CatanConstants.HEX_SIDES, building.getComponentID());
                        }
                        // check neighbour #2
                        if (Arrays.stream(neighbourCoords[1]).max().getAsInt() < board.length &&
                                Arrays.stream(neighbourCoords[1]).min().getAsInt() >= 0) {
                            board[neighbourCoords[1][0]][neighbourCoords[1][1]].setVertexBoardNodeID((vertex + 4) % CatanConstants.HEX_SIDES, building.getComponentID());
                        }
                    }
                }
            }
            ...
            for (CatanTile tile : catanTiles) {
                ...
                if (!(tile.getTileType() == CatanTile.TileType.DESERT || tile.getTileType() == CatanTile.TileType.SEA)) {
                    for (int i = 0; i < CatanConstants.HEX_SIDES; i++) {
                        Edge edge = graph.addConnection(tile.getVerticesBoardNodeIDs()[i], tile.getVerticesBoardNodeIDs()[(i + 1) % CatanConstants.HEX_SIDES]);
                        ...
                        if (Arrays.stream(otherCoords).max().getAsInt() < board.length &&
                            ...
                            neighbour.setEdgeID((i+3) % CatanConstants.HEX_SIDES, edge.getComponentID());
                        }
                    }
                }
            }
        ...
        for (int i = 0; i < CatanConstants.HEX_SIDES; i++) {
            for (int j = 0; j < board.length / 2; j++) {
                ...
                if (counter % 2 == 0 && harbors.size() > 0) {
                    ...
                    ((Building)graphBoard.getNodeByID(tile.getVerticesBoardNodeIDs()[(i + 2) % CatanConstants.HEX_SIDES])).setHarbour(harbour);
                    ((Building)graphBoard.getNodeByID(tile.getVerticesBoardNodeIDs()[(i + 3) % CatanConstants.HEX_SIDES])).setHarbour(harbour);
                }
                counter++;
            }
        }
