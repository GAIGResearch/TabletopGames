...
public class PandemicGameState extends AbstractGameStateWithTurnOrder implements IFeatureRepresentation {
    ...
    public double[] getDistanceFeatures(int playerId) {
        ...
        double[] features = new double[3 + PandemicConstants.colors.length];
        ...
        int nCardsPlayerDeck = ((Deck<Card>)getComponent(PandemicConstants.playerDeckHash)).getSize();
        int nOutbreaks = ((Counter)getComponent(PandemicConstants.outbreaksHash)).getValue();
    ...
    public HashMap<HashMap<Integer, Double>, GameResult> getTerminalFeatures(int playerId) {
        ...
        terminals.put(new HashMap<Integer, Double>() {{ put(0, (double) PandemicConstants.colors.length); }}, GameResult.WIN_GAME);
    ...
    public String getPlayerRole(int i) {
        Card playerCard = ((Card) getComponent(PandemicConstants.playerCardHash, i));
        ...
        for(int key : areas.keySet())
        {
            ...
            if (playerId != -1 && key == -1) {
                ...
                for (Map.Entry<Integer, Component> e: oldComponents.entrySet()) {
                    if (gs.getCoreGameParameters().partialObservable && (e.getKey() == PandemicConstants.playerDeckHash || e.getKey() == PandemicConstants.infectionHash)) {
                        Deck<Card> hiddenDeck = (Deck<Card>) e.getValue().copy();
                        if (gamePhase == Forecast && e.getKey() == PandemicConstants.infectionHash) {
