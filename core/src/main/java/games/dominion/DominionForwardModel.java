...
public class DominionForwardModel extends StandardForwardModel {
            ...
            switch (state.getGamePhase().toString()) {
                case "Play":
                    if (state.actionsLeftForCurrentPlayer < 1 || action instanceof EndPhase) {
                        ...
                        processDelayedActions(DominionConstants.TriggerType.StartBuy, state);
    ...
    private void processDelayedActions(DominionConstants.TriggerType trigger, DominionGameState state) {
        ...
        switch (state.getGamePhase().toString()) {
            case "Play":
                if (state.actionsLeft() > 0) {
                    List<DominionCard> actionCards = state.getDeck(DominionConstants.DeckType.HAND, playerID).stream()
