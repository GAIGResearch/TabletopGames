...
public class DominionHeuristic extends TunableParameters implements IStateHeuristic {
    ...
    public double evaluateState(AbstractGameState gs, int playerId) {
        ...
        if (treasureInHand != 0.0)
            retValue += treasureInHand * Math.min(state.getTotal(playerId, DominionConstants.DeckType.HAND, DominionCard::treasureValue) / 20.0, 1.0);

        // actionCardsInHand - number / 5 of actionCards In Hand
        if (actionCardsInHand != 0.0)
            retValue += actionCardsInHand * Math.min(state.getTotal(playerId, DominionConstants.DeckType.HAND, c -> c.isActionCard() ? 1 : 0) / 5.0, 1.0);
