...
public class DominionGameState extends AbstractGameState implements IPrintable {
    ...
    public void addCard(CardType type, int playerId, DominionConstants.DeckType deckType) {
    ...
    public boolean drawCard(int playerId) {
        return drawCard(playerId, DominionConstants.DeckType.DRAW, playerId, DominionConstants.DeckType.HAND);
    }

    public boolean drawCard(int fromPlayer, DominionConstants.DeckType fromDeck, int toPlayer, DominionConstants.DeckType toDeck) {
        Deck<DominionCard> source = getDeck(fromDeck, fromPlayer);
        Deck<DominionCard> destination = getDeck(toDeck, toPlayer);
        if (source.getSize() == 0) {
            // do stuff
            if (fromDeck == DominionConstants.DeckType.DRAW) {
                Deck<DominionCard> discard = getDeck(DominionConstants.DeckType.DISCARD, fromPlayer);
    ...
    public boolean moveCard(CardType type, int fromPlayer, DominionConstants.DeckType fromDeck, int toPlayer, DominionConstants.DeckType toDeck) {
    ...
    public boolean moveCard(DominionCard cardToMove, int fromPlayer, DominionConstants.DeckType fromDeck, int toPlayer, DominionConstants.DeckType toDeck) {
    ...
    public Deck<DominionCard> getDeck(DominionConstants.DeckType deck, int playerId) {
        switch (deck) {
            case HAND:
    ...
    public int cardsOfType(CardType type, int playerId, DominionConstants.DeckType deck) {
    ...
    public int getTotal(int playerId, DominionConstants.DeckType deck, Function<DominionCard, Integer> cardValuer) {
    ...
    public String toString() {
        ...
        for (int p = 0; p < getNPlayers(); p++) {
            ...
            retValue.append(getDeck(DominionConstants.DeckType.TABLE, p).stream().map(Card::toString).collect(Collectors.joining(", ")));
            retValue.append("\nHand:\n\t");
            retValue.append(getDeck(DominionConstants.DeckType.HAND, p).stream().map(Card::toString).collect(Collectors.joining(", ")));
            retValue.append("\n");
        }
        retValue.append("\n\nHistory:\n\t");
