...
public class BlackjackDeckView extends ComponentView {
    ...
    public BlackjackDeckView(Deck<FrenchCard> d, String dataPath){
        super(d, BlackjackGUIManager.playerWidth, BlackjackGUIManager.cardHeight);
    ...
    protected void paintComponent(Graphics g) {
        drawDeck((Graphics2D)g, new Rectangle(0, 0, width, BlackjackGUIManager.cardHeight));
    }
    ...
    public void drawDeck(Graphics2D g, Rectangle rect){
        ...
        if (deck != null){
            int offset = Math.max((rect.width- BlackjackGUIManager.cardWidth) / deck.getSize(), minimumCardOffset);
            rects = new Rectangle[deck.getSize()];
            for (int i = 0; i < deck.getSize(); i++){
                FrenchCard card = deck.get(i);
                Image cardFace = getCardImage(card);
                Rectangle r = new Rectangle(rect.x + offset * i, rect.y, BlackjackGUIManager.cardWidth, BlackjackGUIManager.cardHeight);
                rects[i] = r;
            ...
            g.drawString(""+deck.getSize(), rect.x+10, rect.y+ BlackjackGUIManager.cardHeight - size);
        }
    }
