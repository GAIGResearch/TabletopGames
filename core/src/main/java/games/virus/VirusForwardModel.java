...
public class VirusForwardModel extends StandardForwardModel {
    ...
    private void createCards(VirusGameState vgs) {
        ...
        for (VirusCard.OrganType organ : VirusCard.OrganType.values()) {
            if (organ != VirusCard.OrganType.Wild && organ != VirusCard.OrganType.Treatment) {
        ...
        for (int i = 0; i < vgp.nCardsPerWildOrgan; i++)
            vgs.drawDeck.add(new VirusCard(VirusCard.OrganType.Wild, VirusCard.VirusCardType.Organ));

        for (int i = 0; i < vgp.nCardsPerWildVirus; i++)
            vgs.drawDeck.add(new VirusCard(VirusCard.OrganType.Wild, VirusCard.VirusCardType.Virus));

        for (int i = 0; i < vgp.nCardsPerWildMedicine; i++)
            vgs.drawDeck.add(new VirusCard(VirusCard.OrganType.Wild, VirusCard.VirusCardType.Medicine));
                    ...
                    if (otherPlayerId != playerId) {
                        ...
                        for (VirusCard.OrganType otherOrganType : VirusCard.OrganType.values()) {
                            if (isWild) {
                                ...
                                if (otherBody.hasOrganNeutral(otherOrganType) && (myOrganType == otherOrganType || otherOrganType == VirusCard.OrganType.Wild)) {
                                    actions.add(new PlaySpreading(playerHand.getComponentID(),
                                            gameState.discardDeck.getComponentID(),
                                            cardIdx,
                                            myBody.getComponentID(),
