    ...
    protected void paintComponent(Graphics g1) {
        ...
        for (TMTypes.Resource res: TMTypes.Resource.values()) {
            if (res.isPlayerBoardRes()) {
                ...
                drawShadowStringCentered(g, "" + gs.getPlayerResources()[TMGUI.focusPlayer].get(res).getValue(),
                ...
                drawShadowStringCentered(g, "" + gs.getPlayerProduction()[TMGUI.focusPlayer].get(res).getValue(),
                ...
                g.setColor(playerColors[TMGUI.focusPlayer]);
        ...
        for (TMTypes.Tag t: TMTypes.Tag.values()) {
            int nCards = gs.getPlayerCardsPlayedTags()[TMGUI.focusPlayer].get(t).getValue();
        ...
        for (TMTypes.CardType t: TMTypes.CardType.values()) {
            if (t.isPlayableStandard()) {
                int nCards = gs.getPlayerCardsPlayedTypes()[TMGUI.focusPlayer].get(t).getValue();
                text = t.name() + ": " + nCards;
        ...
        for (TMTypes.Tile t: TMTypes.Tile.values()) {
            int nTiles = gs.getPlayerTilesPlaced()[TMGUI.focusPlayer].get(t).getValue();
            Image img = ImageIO.GetInstance().getImage(t.getImagePath());
            drawImage(g, img, startX + k*spacing/2 + k*2*defaultItemSize/3, startY, defaultItemSize/3, defaultItemSize/3);
        ...
        for (Milestone m: gs.getMilestones()) {
            ...
            int progress = m.checkProgress(gs, TMGUI.focusPlayer);
        ...
        for (Award a: gs.getAwards()) {
            ...
            int progress = a.checkProgress(gs, TMGUI.focusPlayer);
