...
public class ToadMCTSPlayer extends MCTSPlayer {
    ...
    public ToadMCTSPlayer(MCTSParams params) {
        super(params);
        if (params.opponentTreePolicy == MCTSEnums.OpponentTreePolicy.MultiTree && params.reuseTree) {
    ...
    private boolean functionalityApplies(AbstractGameState gameState) {
        ...
        if (params.opponentTreePolicy == MCTSEnums.OpponentTreePolicy.SelfOnly || params.opponentTreePolicy == MCTSEnums.OpponentTreePolicy.MCGSSelfOnly) {
            return false;
        }
