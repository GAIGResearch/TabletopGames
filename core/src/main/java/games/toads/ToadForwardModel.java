...
public class ToadForwardModel extends StandardForwardModel {
    ...
    protected void _setup(AbstractGameState firstState) {
        ...
        if (params.discardOption)
            state.setGamePhase(ToadConstants.ToadGamePhase.DISCARD);
        else
            state.setGamePhase(ToadConstants.ToadGamePhase.PLAY);
    }

    @Override
    protected List<AbstractAction> _computeAvailableActions(AbstractGameState gameState) {
        ToadGameState state = (ToadGameState) gameState;
        if (state.getGamePhase().equals(ToadConstants.ToadGamePhase.DISCARD)) {
            return computeDiscardActions(state);
        } else if (state.getGamePhase().equals(ToadConstants.ToadGamePhase.PLAY)) {
    ...
    protected void _afterAction(AbstractGameState gameState, AbstractAction action) {
        ...
        ToadGameState state = (ToadGameState) gameState;
        if (state.getGamePhase() == ToadConstants.ToadGamePhase.DISCARD) {
        ...
        if (state.hiddenFlankCards[currentPlayer] == null) {
            ...
            if (battle.getPostBattleActions().isEmpty()) {
                ...
                state.setGamePhase(ToadConstants.ToadGamePhase.POST_BATTLE);
