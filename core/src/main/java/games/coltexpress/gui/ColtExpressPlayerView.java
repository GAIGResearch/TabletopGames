...
public class ColtExpressPlayerView extends JComponent {
    ...
    public ColtExpressPlayerView(int playerId, String dataPath,
        ...
        this.width = ColtExpressGUIManager.playerAreaWidth + border*2;
        this.height = ColtExpressGUIManager.playerAreaHeight + border;
    ...
    protected void paintComponent(Graphics g) {
        ...
        g.drawImage(card, border, border, ColtExpressGUIManager.ceCardWidth, ColtExpressGUIManager.ceCardHeight, null);
        g.setColor(Color.black);
        g.drawString(playerCard.name(), border+ ColtExpressGUIManager.ceCardWidth/2-20, border+ ColtExpressGUIManager.ceCardHeight-5);

        // Draw loot, bullets left, points total if game end
        playerLoot.drawDeck((Graphics2D) g, new Rectangle(border+ ColtExpressGUIManager.ceCardWidth + 10, border,
                defaultItemSize*2, defaultItemSize), false, 1.0);
        g.setColor(Color.black);
        g.drawString("Bullets left: " + bulletsLeft, border, border+ ColtExpressGUIManager.ceCardHeight + 15);
        if (gameEnd) {
            if (lootSum == -1) {
                lootSum = 0;
            ...
            g.drawString(endResult, border+ ColtExpressGUIManager.ceCardWidth*2 + 15, border+ ColtExpressGUIManager.ceCardHeight + 15);
        }

        // Draw player deck
        g.drawImage(cardBack, border+ ColtExpressGUIManager.ceCardWidth + defaultItemSize*2 + 15, border, ColtExpressGUIManager.ceCardWidth, ColtExpressGUIManager.ceCardHeight, null);
        if (playerDeck != null) {
            g.drawString("" + playerDeck.getSize(), border+ ColtExpressGUIManager.ceCardWidth + defaultItemSize*2 + 20, ColtExpressGUIManager.ceCardHeight);
        }

        // Draw player hand
        playerHand.drawDeck((Graphics2D) g, new Rectangle(border+ ColtExpressGUIManager.ceCardWidth + defaultItemSize*2 + 15+ ColtExpressGUIManager.ceCardWidth + 30, border,
                width- ColtExpressGUIManager.ceCardWidth*2-defaultItemSize*2-45, ColtExpressGUIManager.ceCardHeight), false, 1.0);
    }
