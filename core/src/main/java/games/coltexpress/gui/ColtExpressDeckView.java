...
public class ColtExpressDeckView<T extends Component> extends ComponentView {
    ...
    public ColtExpressDeckView(Deck<T> d, boolean visible, String dataPath,
                               HashMap<Integer, ColtExpressTypes.CharacterType> characters) {
        super(d, ColtExpressGUIManager.playerAreaWidth, ColtExpressGUIManager.ceCardHeight);
        this.front = visible;
        ...
        if (deck != null && deck.getSize() > 0) {
            ...
            while (i >= 0 && !firstOnTop || i < deck.getSize() && firstOnTop) {
                if (deck.get(0) instanceof ColtExpressCard) {
                    // Player card
                    int offset = (rect.width- ColtExpressGUIManager.ceCardWidth) / deck.getSize();
                    Rectangle r = new Rectangle(rect.x + offset * i, rect.y, (int)(ColtExpressGUIManager.ceCardWidth * scale), (int)(ColtExpressGUIManager.ceCardHeight * scale));
                    rects[i] = r;
                    ...
                    // Round card
                    int offset = (rect.width-(int)(ColtExpressGUIManager.roundCardWidth*scale)) / deck.getSize();
                    Rectangle r = new Rectangle(rect.x + offset * i, rect.y, (int)(ColtExpressGUIManager.roundCardWidth*scale), (int)(ColtExpressGUIManager.roundCardHeight*scale));
                    rects[i] = r;
            ...
            if (cardHighlight != -1) {
                ...
                if (deck.get(0) instanceof ColtExpressCard) {
                    // Player card
                    int offset = (rect.width- ColtExpressGUIManager.ceCardWidth) / deck.getSize();
                    Rectangle r = new Rectangle(rect.x + offset * cardHighlight, rect.y, ColtExpressGUIManager.ceCardWidth, ColtExpressGUIManager.ceCardHeight);
                    ...
                    // Round card
                    int offset = (rect.width-(int)(ColtExpressGUIManager.roundCardWidth*scale)) / deck.getSize();
                    Rectangle r = new Rectangle(rect.x + offset * cardHighlight, rect.y, (int)(ColtExpressGUIManager.roundCardWidth*scale), (int)(ColtExpressGUIManager.roundCardHeight*scale));
