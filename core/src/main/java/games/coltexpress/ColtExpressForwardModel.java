...
public class ColtExpressForwardModel extends StandardForwardModelWithTurnOrder {
    ...
    public void _setup(AbstractGameState firstState) {
        ...
        HashSet<ColtExpressTypes.CharacterType> characters = new HashSet<>();
        Collections.addAll(characters, ColtExpressTypes.CharacterType.values());
        ...
        for (int playerIndex = 0; playerIndex < cegs.getNPlayers(); playerIndex++) {
            ...
            ColtExpressTypes.CharacterType characterType = pickRandomCharacterType(rndForCharacters, characters);
            cegs.playerCharacters.put(playerIndex, characterType);
            if (characterType == ColtExpressTypes.CharacterType.Belle)
            ...
            for (Group<ColtExpressTypes.LootType, Integer, Integer> e : cep.playerStartLoot) {
                ColtExpressTypes.LootType lootType = e.a;
                int value = e.b;
                int nLoot = e.c;
        ...
        firstState.setGamePhase(ColtExpressGameState.ColtExpressGamePhase.PlanActions);
    ...
    private ColtExpressTypes.CharacterType pickRandomCharacterType(Random rnd, HashSet<ColtExpressTypes.CharacterType> characters) {
        int size = characters.size();
        int item = rnd.nextInt(size);
        int i = 0;
        for (ColtExpressTypes.CharacterType obj : characters) {
            if (i == item) {
...
//            actions = drawAction();
    ...
    private ArrayList<AbstractAction> schemingActions(ColtExpressGameState cegs) {
        ...
        for (int i = 0; i < playerHand.getSize(); i++) {
            ...
            boolean hidden = ceto.isHiddenTurn() ||
                    (cegs.playerCharacters.get(player) == ColtExpressTypes.CharacterType.Ghost && ceto.getFullPlayerTurnCounter() == 0);
                ...
                case CollectMoney:
                    for (Compartment compartment : cegs.trainCompartments) {
                        ...
                        if (availableLoot != null && availableLoot.getSize() > 0) {
                            HashSet<ColtExpressTypes.LootType> lootTypes = new HashSet<>();
                            for (Loot loot : availableLoot.getComponents()) {
                                lootTypes.add(loot.getLootType());
                            }
                            for (ColtExpressTypes.LootType lt : lootTypes) {
    ...
    private void createPunchingActions(ColtExpressGameState cegs, ArrayList<AbstractAction> actions, int player, int cardIdx) {
        int deckFromID = cegs.plannedActions.getComponentID();
        int deckToID = cegs.playerDecks.get(player).getComponentID();
        boolean playerIsCheyenne = cegs.playerCharacters.get(player) == ColtExpressTypes.CharacterType.Cheyenne;
    ...
    private void createShootingActions(ColtExpressGameState cegs, ArrayList<AbstractAction> actions, int player, int cardIdx) {
        int deckFromID = cegs.plannedActions.getComponentID();
        int deckToID = cegs.playerDecks.get(player).getComponentID();
        boolean playerIsDjango = cegs.playerCharacters.get(player) == ColtExpressTypes.CharacterType.Django;
        ...
        if (playerCompartment != null) {
            ...
            if (playerOnTop) {
                ...
                if (cegs.playerCharacters.get(player) == ColtExpressTypes.CharacterType.Tuco) {
