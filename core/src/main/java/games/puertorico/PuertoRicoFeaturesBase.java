...
public class PuertoRicoFeaturesBase implements IStateFeatureVector {
    ...
    public double[] featureVector(AbstractGameState gs, int playerID) {
        ...
        retValue[4] = state.getPlayerBoard(playerID).getPlantations().stream().filter(p -> p.crop == PuertoRicoConstants.Crop.QUARRY).count();
        ...
        int goodTypes = 0;
        int goodVolume = 0;
        Map<PuertoRicoConstants.Crop, Long> plantationsByCrop = state.getPlayerBoard(playerID).getPlantations().stream()
                .filter(Plantation::isOccupied)
                .collect(groupingBy(p -> p.crop, Collectors.counting()));
        Map<PuertoRicoConstants.Crop, Integer> productionByCrop = state.getPlayerBoard(playerID).getBuildings().stream()
        ...
        for (PuertoRicoConstants.Crop c : plantationsByCrop.keySet()) {
            if (c == PuertoRicoConstants.Crop.CORN && plantationsByCrop.get(c) > 0) {
                goodTypes++;
