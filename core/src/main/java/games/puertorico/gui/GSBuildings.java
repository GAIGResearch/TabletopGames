...
public class GSBuildings extends JComponent {
    ...
    Dimension size = new Dimension(PRGUIUtils.pad*2+(PRGUIUtils.buildingWidth + PRGUIUtils.pad)*(discounts.size()), 300);
    ...
    protected void paintComponent(Graphics gg) {
        ...
        for (int discount : discounts) {
            int x = PRGUIUtils.pad + col * (PRGUIUtils.buildingWidth + PRGUIUtils.pad);
            ...
            int y = PRGUIUtils.pad;

            // Draw discount at top of column
            g.setColor(PRGUIUtils.secondaryColor);
            g.setFont(PRGUIUtils.textFontBold);
            g.drawString(String.valueOf(discount), x + PRGUIUtils.buildingWidth/2 - fm.stringWidth(String.valueOf(discount))/2, y + PRGUIUtils.textFontSize);
            g.setColor(Color.black);
            y += PRGUIUtils.textFontSize + PRGUIUtils.pad;
            ...
            for (PuertoRicoConstants.BuildingType buildingType: buildings) {
                ...
                PRGUIUtils.drawBuilding(g, b, x, y, nAvailable);
                int size = 1;
                if (b != null) size = b.buildingType.size;
                buildingRectMap.put(new Rectangle(x, y, PRGUIUtils.buildingWidth, PRGUIUtils.buildingHeight * size), b);
                ...
                if (gs.getCurrentRole() == PuertoRicoConstants.Role.BUILDER && gui.getHumanPlayerId().contains(gs.getCurrentPlayer())) {
                    ...
                    if (b != null) {
                        ...
                        if (townSizeCondition && costCondition && fitsOnBoardCondition && uniqueCondition) {
                            ...
                            buildingTooltipUnavailableActionMap.put(new Rectangle(x, y, PRGUIUtils.buildingWidth, PRGUIUtils.buildingHeight * size), tooltip);
                        }
                        g.drawRect(x, y, PRGUIUtils.buildingWidth, PRGUIUtils.buildingHeight * size);
                    }
                    g.setStroke(s);
                }
