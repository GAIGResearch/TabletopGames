...
public class PlayerBoard extends JComponent implements IScreenHighlight {
    ...
    public PlayerBoard(PuertoRicoGUI gui, PuertoRicoGameState gs, int playerId) {
        ...
        size = new Dimension(Math.max(townGridWidth * PRGUIUtils.buildingWidth, nPlantationSlots * PRGUIUtils.plantationSize / 2) + borderPadX*2, 315);
    ...
    protected void paintComponent(Graphics gg) {
        ...
        g.drawString("VP: " + playerBoard.getVP(), PRGUIUtils.pad + borderPadX, PRGUIUtils.textFontSize + PRGUIUtils.pad + borderPadY);
        g.setColor(Color.black);
        g.drawString("Doubloons: " + playerBoard.getDoubloons(), PRGUIUtils.pad + borderPadX, PRGUIUtils.textFontSize*2 + PRGUIUtils.pad*2 + borderPadY);
        g.drawString("Colonists: " + playerBoard.getUnassignedColonists(), PRGUIUtils.pad + borderPadX, PRGUIUtils.textFontSize*3 + PRGUIUtils.pad*3 + borderPadY);

        // Crops in supply
        int x = PRGUIUtils.pad + borderPadX;
        int y = PRGUIUtils.textFontSize*4 + PRGUIUtils.pad*6 + borderPadY;
        for (PuertoRicoConstants.Crop crop: PuertoRicoConstants.Crop.values()) {
            ...
            PRGUIUtils.drawBarrel(g, x, y- PRGUIUtils.textFontSize, PRGUIUtils.cropColorMap.get(crop));
            cropRectMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), crop);

            if (cropClicked == crop) {
                Stroke s = g.getStroke();
                g.setStroke(new BasicStroke(2));
                g.setColor(PRGUIUtils.highlightColor);
                g.drawRect(x- PRGUIUtils.pad, y- PRGUIUtils.textFontSize- PRGUIUtils.pad, PRGUIUtils.barrelWidth+ PRGUIUtils.pad*2, PRGUIUtils.barrelHeight+ PRGUIUtils.pad*2);
                g.setStroke(s);
            }
            ...
            if (highlightCropStore) {
                ...
                if (gs.getCurrentRole() == PuertoRicoConstants.Role.TRADER) {
                    ...
                    if (hasAny && (hasOffice || notInMarket)) {
                        ...
                        if (!hasAny) {
                            invalidActionTooltipMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>No " + crop.name() + " to sell.</li>");
                        }
                        if (!hasOffice && !notInMarket) {
                            invalidActionTooltipMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>" + crop.name() + " is already in the market, and you do not have an office.");
                        }
                    }
                }
                    ...
                    if (playerProduces && inSupply) {
                        ...
                        g.setColor(Color.red);
                        if (!playerProduces) {
                            invalidActionTooltipMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>You do not produce " + crop.name() + ".</li>");
                        }
                        if (!inSupply) {
                            invalidActionTooltipMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>There are no more " + crop.name() + " left in the supply.</li>");
                        }
                    }
                }
                    ...
                    if (gui.shipsAndMarket.shipClicked != -1) {
                        ...
                        if (hasAny && emptyAndClickedUniqueCondition) {
                            ...
                            if (crop != null) {
                                if (!hasAny) {
                                    invalidActionTooltipMap.put(new Rectangle(x, y - PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>You do not have any " + crop.name() + " to load.</li>");
                                }
                                if (!emptyAndClickedUniqueCondition) {
                                    invalidActionTooltipMap.put(new Rectangle(x, y - PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>You cannot load " + crop.name() + " onto the selected ship.</li>");
                                }
                            }
                        }
                    } else {
                        ...
                        if (hasAny && (emptyShipAvailable && uniqueCondition || matchesGoodOnAnotherShipWithCapacity)) {
                            ...
                            if (crop != null) {
                                ...
                                if (!uniqueCondition && !matchesGoodOnAnotherShipWithCapacity) {
                                    invalidActionTooltipMap.put(new Rectangle(x, y - PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>Ships carry " + crop.name() + " already, but those are full.</li>");
                                }
                            }
                        }
                    }
                    ...
                    if (hasAny) {
                        ...
                        g.setColor(Color.red);
                        invalidActionTooltipMap.put(new Rectangle(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight), "<li>You do not have any " + crop.name() + " to store.</li>");
                    }
                }

                Stroke s = g.getStroke();
                g.setStroke(new BasicStroke(2));
                g.drawRect(x, y- PRGUIUtils.textFontSize, PRGUIUtils.barrelWidth, PRGUIUtils.barrelHeight);
                g.setColor(Color.black);
                g.setStroke(s);
            }
        ...
        if (width / townGridWidth < PRGUIUtils.buildingWidth) {
            PRGUIUtils.buildingWidth = width / townGridWidth;
        }
        if (height / townGridHeight < PRGUIUtils.buildingHeight) {
            PRGUIUtils.buildingHeight = height / townGridHeight;
        }
        ...
        // Draw the town grid
        y = PRGUIUtils.textFontSize*4 + PRGUIUtils.pad*6 + PRGUIUtils.barrelHeight + borderPadY;
        for (i = 0; i < townGridHeight; i++) {
            x = borderPadX;
            for (j = 0; j < townGridWidth; j++) {
                if (townGrid[i][j] != -1 && i > 0 && townGrid[i-1][j] == townGrid[i][j]) {
                    // Skip this square
                    x += PRGUIUtils.buildingWidth;
                    continue;
                }
                Building b = null;
                int h = PRGUIUtils.buildingHeight;
                if (townGrid[i][j] != -1) {
                    b = playerBoard.getBuildings().get(townGrid[i][j]);
                    h *= b.buildingType.size;
                }
                PRGUIUtils.drawBuilding(g, b, x, y, 0);
                buildingRectMap.put(new Rectangle(x, y, PRGUIUtils.buildingWidth, h), b);
                ...
                if (highlightPlantationOrBuilding && b != null) {
                    ...
                    if (b.getOccupation() < b.buildingType.capacity) {
                        ...
                        g.setColor(Color.red);
                        invalidActionTooltipMap.put(new Rectangle(x, y, PRGUIUtils.buildingWidth, h), "<li>Can't place any more colonists, building is full.</li>");
                    }
                    g.drawRect(x, y, PRGUIUtils.buildingWidth, h);
                    g.setStroke(s);
                    g.setColor(Color.black);
                }
            ...
            y += PRGUIUtils.buildingHeight;
        }

        // Plantations
        y += PRGUIUtils.pad - PRGUIUtils.plantationSize;
        int nColsP = width / PRGUIUtils.plantationSize;
        x = borderPadX;
        for (i = 0; i < nPlantationSlots; i++) {
            if (i % nColsP == 0) {
                x = borderPadX;
                y += PRGUIUtils.plantationSize;
            }
            Plantation p = null;
            if (i < playerBoard.getPlantations().size()) {
                p = playerBoard.getPlantations().get(i);
            }
            PRGUIUtils.drawPlantation(g, p, x, y);
            plantationRectMap.put(new Rectangle(x, y, PRGUIUtils.plantationSize, PRGUIUtils.plantationSize), p);
            ...
            if (highlightPlantationOrBuilding && p != null) {
                ...
                if (!p.isOccupied()) {
                    ...
                    g.setColor(Color.red);
                    invalidActionTooltipMap.put(new Rectangle(x, y, PRGUIUtils.plantationSize, PRGUIUtils.plantationSize), "<li>Can't place a colonist here, plantation is already occupied.</li>");
                }
                g.drawRect(x, y, PRGUIUtils.plantationSize, PRGUIUtils.plantationSize);
                g.setStroke(s);
                g.setColor(Color.black);
            }

            x += PRGUIUtils.plantationSize;
        }
    }
