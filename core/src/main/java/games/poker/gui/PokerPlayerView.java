...
public class PokerPlayerView extends PokerDeckView {
    ...
    public PokerPlayerView(Deck<FrenchCard> d, int playerId, String dataPath) {
        ...
        this.width = PokerGUIManager.playerAreaWidth + border*2;
        this.height = PokerGUIManager.playerAreaHeight + border + borderBottom;
    ...
    protected void paintComponent(Graphics g) {
        drawDeck((Graphics2D) g, new Rectangle(border, border, PokerGUIManager.playerAreaWidth, PokerGUIManager.pokerCardHeight));
        g.setColor(new Color(87, 37, 25));
        Font f = g.getFont();
        g.setFont(new Font(f.getName(), Font.BOLD, 30));
        if (firstPlayerOfRound) {
            g.drawString("*", border + PokerGUIManager.playerAreaWidth / 2 - 20, border + PokerGUIManager.pokerCardHeight + 25);
        }
        g.drawString("" + bet, PokerGUIManager.playerAreaWidth - 50, PokerGUIManager.playerAreaHeight/2);
        g.setFont(f);
    }
