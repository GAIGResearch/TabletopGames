...
public class PokerForwardModel extends StandardForwardModel {
    ...
    private void setupRound(PokerGameState pgs) {
        ...
        pgs.setGamePhase(PokerGameState.PokerGamePhase.Preflop);
        pgs.setBet(false);
    }
    ...
    protected void _afterAction(AbstractGameState gameState, AbstractAction action) {
        ...
        if (!pgs.playersLeftToAct()) {
            ...
            if (pgs.getGamePhase() == PokerGameState.PokerGamePhase.Preflop) {
                // Add flop
                for (int i = 0; i < pgp.nFlopCards; i++) {
                    pgs.communityCards.add(pgs.drawDeck.draw());
                }
                pgs.setGamePhase(PokerGameState.PokerGamePhase.Flop);
            } else if (pgs.getGamePhase() == PokerGameState.PokerGamePhase.Flop) {
                // Add turn
                for (int i = 0; i < pgp.nTurnCards; i++) {
                    pgs.communityCards.add(pgs.drawDeck.draw());
                }
                pgs.setGamePhase(PokerGameState.PokerGamePhase.Turn);
            } else if (pgs.getGamePhase() == PokerGameState.PokerGamePhase.Turn) {
                // Add river
                for (int i = 0; i < pgp.nRiverCards; i++) {
                    pgs.communityCards.add(pgs.drawDeck.draw());
                }
                pgs.setGamePhase(PokerGameState.PokerGamePhase.River);
            } else if (pgs.getGamePhase() == PokerGameState.PokerGamePhase.River) {
                // Round is over
                roundEnd(pgs);
                return;
            }
