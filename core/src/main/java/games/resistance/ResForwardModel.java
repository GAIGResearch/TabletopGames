...
public class ResForwardModel extends StandardForwardModel {
    ...
    protected void _setup(AbstractGameState firstState) {
        ...
        resgs.leaderID = 0;
        resgs.setGamePhase(ResGameState.ResGamePhase.LeaderSelectsTeam);
    ...
    protected List<AbstractAction> _computeAvailableActions(AbstractGameState gameState) {
        ...
        if (resgs.getGamePhase() == ResGameState.ResGamePhase.LeaderSelectsTeam) {
        ...
        } else if (resgs.getGamePhase() == ResGameState.ResGamePhase.TeamSelectionVote) {
        ...
        } else if (resgs.getGamePhase() == ResGameState.ResGamePhase.MissionVote) {
    ...
    protected void _afterAction(AbstractGameState currentState, AbstractAction action) {
        ...
        //Leader Selects Team
        if (resgs.getGamePhase() == ResGameState.ResGamePhase.LeaderSelectsTeam) {
            resgs.setGamePhase(ResGameState.ResGamePhase.TeamSelectionVote);
            endPlayerTurn(resgs);
        } else if (resgs.getGamePhase() == ResGameState.ResGamePhase.TeamSelectionVote) {
            ...
            resgs.logEvent(Event.GameEvent.GAME_EVENT, "Vote was " + Arrays.stream(resgs.votingChoice).map(Objects::toString).collect(Collectors.joining(",")));
            resgs.clearVoteChoices();

            if (resgs.voteSuccess) {
                resgs.finalTeamChoice = resgs.teamChoice;
                resgs.setGamePhase(ResGameState.ResGamePhase.MissionVote);
...
;
    ...
    void revealCards(ResGameState resgs) {
        if (resgs.getGamePhase() == ResGameState.ResGamePhase.TeamSelectionVote) {
        ...
        if (resgs.getGamePhase() == ResGameState.ResGamePhase.MissionVote) {
