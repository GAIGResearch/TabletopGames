...
public class BattleloreBoardView extends ComponentView {
    ...
    public void drawGridBoard(Graphics2D g, GridBoard<MapTile> gridBoard, int x, int y) {
        ...
        for (int i = 0; i < gridBoard.getHeight(); i++) {
            for (int j = 0; ((i % 2 == 0 && j < gridBoard.getWidth()) || (i % 2 == 1 && j < gridBoard.getWidth()-1)); j++) {
                ...
                int xC = x + 35 + j * GUI.defaultItemSize;
                int yC = (int) (y + offSetY + i * Math.round(GUI.defaultItemSize / 1.2));
    ...
    private void drawCell(Graphics2D g, MapTile tile, int x, int y) {
        ...
        if (tile != null) {
            ...
            for (int i = 0; i < 6; i++) {
                h.addPoint((int) (x + GUI.defaultItemSize/1.75 * Math.cos(Math.PI/2 + i * 2 * Math.PI / 6)),
                        (int) (y + GUI.defaultItemSize/1.75 * Math.sin(Math.PI/2 + i * 2 * Math.PI / 6)));
            }
            ...
            drawElementName(g,x - GUI.defaultItemSize/3, y, tile);
        }
    }
