...
public class PartialObservableDeck<T extends Component> extends Deck<T> {
    ...
    public PartialObservableDeck(String id, int ownerID, boolean[] defaultVisibility) {
        super(id, ownerID, CoreConstants.VisibilityMode.MIXED_VISIBILITY);
        this.deckVisibility = defaultVisibility;
    }

    public PartialObservableDeck(String id, int ownerID, int nPlayers, CoreConstants.VisibilityMode visibilityMode) {
    ...
    private PartialObservableDeck(String name, int ownerID, boolean[] defaultVisibility, int ID) {
        super(name, ownerID, ID, CoreConstants.VisibilityMode.MIXED_VISIBILITY);
        this.deckVisibility = defaultVisibility;
    }
    ...
    @Override
    public void setVisibility(CoreConstants.VisibilityMode visibilityMode) {
    ...
    private void applyVisibilityMode() {
        if (getVisibilityMode() == CoreConstants.VisibilityMode.TOP_VISIBLE_TO_ALL)
            for (int j = 0; j < deckVisibility.length; j++)
                elementVisibility.get(0)[j] = true;
        if (getVisibilityMode() == CoreConstants.VisibilityMode.BOTTOM_VISIBLE_TO_ALL)
            for (int j = 0; j < deckVisibility.length; j++)
