...
public abstract class StandardForwardModel extends AbstractForwardModel {
    ...
    public final void endPlayerTurn(AbstractGameState gs, int nextPlayer) {
        if (gs.getGameStatus() != CoreConstants.GameResult.GAME_ONGOING) return;

        int currentPlayer = gs.getCurrentPlayer();
        gs.getPlayerTimer()[currentPlayer].incrementTurn();
        gs.listeners.forEach(l -> l.onEvent(Event.createEvent(Event.GameEvent.TURN_OVER, gs, currentPlayer)));
        if (gs.getCoreGameParameters().recordEventHistory) {
            gs.recordHistory(Event.GameEvent.TURN_OVER.name());
        }
        gs.turnCounter++;
    ...
    public final void endRound(AbstractGameState gs, int firstPlayerOfNextRound) {
        ...
        gs.listeners.forEach(l -> l.onEvent(Event.createEvent(Event.GameEvent.ROUND_OVER, gs, currentPlayer)));
        if (gs.getCoreGameParameters().recordEventHistory) {
        ...
        if (gs.getGameParameters().maxRounds != -1 && gs.roundCounter == gs.getGameParameters().maxRounds) {
            ...
            gs.setGameStatus(CoreConstants.GameResult.TIMEOUT);
            Arrays.fill(gs.playerResults, CoreConstants.GameResult.TIMEOUT);
        } else {
            gs.turnCounter = 0;
