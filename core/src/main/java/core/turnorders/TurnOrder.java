...
public abstract class TurnOrder {
    ...
    public final void moveToNextPlayer(AbstractGameState gameState, int newTurnOwner) {
        ...
        int n = 0;
        while (gameState.getPlayerResults()[turnOwner] != CoreConstants.GameResult.GAME_ONGOING) {
    ...
    public final void endGame(AbstractGameState gs) {
        ...
        for (int p = 0; p < gs.getNPlayers(); p++) {
            ...
            if (o == 1 && drawn)
                gs.setPlayerResult(CoreConstants.GameResult.DRAW_GAME, p);
            else if (o == 1)
                gs.setPlayerResult(CoreConstants.GameResult.WIN_GAME, p);
            else
