...
public class PartialObervableDecks {
    ...
    public void addingElementToDeckPicksUpDefaultVisibility() {
        ...
        for (int i = 0; i < 2; i++) {
            Assert.assertTrue(deck.isComponentVisible(i, 0));
            Assert.assertFalse(deck.isComponentVisible(i, 1));
            Assert.assertFalse(deck.isComponentVisible(i, 2));
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (i != 2) {
                ...
                Assert.assertTrue(deck.isComponentVisible(i, 0));
                Assert.assertTrue(deck.isComponentVisible(i, 1));
                Assert.assertTrue(deck.isComponentVisible(i, 2));
            }
        }
    }
    ...
    public void initialisingWithHiddenToAllDoes() {
        ...
        for (int i = 0; i < 3; i++) {
            Assert.assertFalse(deck.isComponentVisible(i, 0));
            Assert.assertFalse(deck.isComponentVisible(i, 1));
            Assert.assertFalse(deck.isComponentVisible(i, 2));
            Assert.assertFalse(deck.isComponentVisible(i, 3));
        }
        ...
        for (int i = 0; i < 3; i++) {
            if (i == 0) {
                ...
                Assert.assertTrue(deck.isComponentVisible(i, 1));
                Assert.assertFalse(deck.isComponentVisible(i, 2));
                Assert.assertTrue(deck.isComponentVisible(i, 3));
            } else {
                Assert.assertFalse(deck.isComponentVisible(i, 0));
                Assert.assertFalse(deck.isComponentVisible(i, 1));
                Assert.assertFalse(deck.isComponentVisible(i, 2));
    ...
    public void allVisibleIfDealtOneAtATimeAndTopMostIsVisible() {
        ...
        for (int i = 0; i < 3; i++) {
            for (int p = 0; p < 4; p++) {
                ...
                Assert.assertTrue(deck.isComponentVisible(i, p));
            }
        }
    }
        ...
        for (int i = 0; i < 3; i++) {
            for (int p = 0; p < 4; p++) {
                ...
                if (i == 0) {
                    ...
                    Assert.assertFalse(deck.isComponentVisible(i, p));
                }
            }
        }
    }
    ...
    public void addingWholeDeckDoesNotIncreaseTopVisibility() {
        ...
        deck2.add(deck);
        for (int p = 0; p < 4; p++) {
            Assert.assertTrue(deck2.isComponentVisible(0, p));
            Assert.assertFalse(deck2.isComponentVisible(1, p));
            Assert.assertFalse(deck2.isComponentVisible(2, p));
            Assert.assertTrue(deck2.isComponentVisible(3, p));
        }
        ...
        for (int p = 0; p < 4; p++) {
            ...
            Assert.assertFalse(deck2.isComponentVisible(1, p));
            Assert.assertFalse(deck2.isComponentVisible(2, p));
            Assert.assertFalse(deck2.isComponentVisible(3, p));
        }
    }
    ...
    public void setVisibilityModeToTopAfterCreatingDeck() {
        ...
        for (int p = 0; p < 4; p++) {
            Assert.assertTrue(deck.isComponentVisible(0, p));
            Assert.assertFalse(deck.isComponentVisible(1, p));
            Assert.assertFalse(deck.isComponentVisible(2, p));
            Assert.assertFalse(deck.isComponentVisible(3, p));
        }
    }
    ...
    public void reshuffleTopDeckWithBottomDeck() {
        ...
        for (int p = 0; p < 4; p++) {
            ...
            Assert.assertFalse(deck.isComponentVisible(2, p));
            Assert.assertFalse(deck.isComponentVisible(3, p));
            Assert.assertFalse(deck.isComponentVisible(4, p));
            Assert.assertTrue(deck.isComponentVisible(5, p));
        }

        deck.redeterminiseUnknown(rnd, 1);
        assertEquals(CardType.SILVER, deck.get(0).cardType());
        Assert.assertFalse(deck.get(1).cardType() == CardType.GOLD &&
    ...
    // There is a boolean array equals assertion in JUNIT 4.12+
    // This is a placeholder.
    private void assertEqualsBooleanArray(boolean[] first, boolean[] second)
    {
    ...
    public void reshuffleSingleDeckWithPerspective() {
        ...
        assertEqualsBooleanArray(new boolean[]{true, false, true, false}, deck2.getVisibilityOfComponent(5));
        assertEquals(CardType.GOLD, deck2.get(4).cardType());
        assertEqualsBooleanArray(new boolean[]{true, false, false, false}, deck2.getVisibilityOfComponent(4));
        assertEqualsBooleanArray(new boolean[]{false, false, false, false}, deck2.getVisibilityOfComponent(3));
        assertEqualsBooleanArray(new boolean[]{false, true, false, false}, deck2.getVisibilityOfComponent(2));

        deck2.redeterminiseUnknown(rnd, 2);
        assertEquals(CardType.MERCHANT, deck2.get(5).cardType());
        assertEqualsBooleanArray(new boolean[]{true, false, true, false}, deck2.getVisibilityOfComponent(5));
        assertEqualsBooleanArray(new boolean[]{true, false, false, false}, deck2.getVisibilityOfComponent(4));
        assertNotSame(CardType.GOLD, deck2.get(4).cardType());
        assertNotSame(CardType.SILVER, deck2.get(3).cardType());
        ...
        assertNotSame(card3, deck2.get(3).cardType());
    ...
    public void reshuffleTwoDecks() {
        ...
        assertNotSame(next1, deck.get(1).cardType());
        assertNotSame(next2, deck2.get(1).cardType());
    }
    ...
    public void shuffleDeckAndPartialObservableDeck() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            assertEqualsBooleanArray(new boolean[]{false, false, true, false}, deck.getVisibilityOfComponent(1));
            assertEqualsBooleanArray(new boolean[]{true, true, true, true}, deck.getVisibilityOfComponent(0));
            assertEqualsBooleanArray(new boolean[]{false, false, false, false}, deck.getVisibilityOfComponent(2));
        ...
        Assert.assertEquals(1, deckCard2.stream().filter(c -> c == card2).count(), 2);
        Assert.assertEquals(1, deck2Card1.stream().filter(c -> c == card1_2).count(), 2);
    }
    ...
    public void shuffleTwoDecksWithPredicate() {
        ...
        Assert.assertEquals(3, nonShuffledCount[0], 3); // 2 cards, so expect 20/6 = 3
        Assert.assertEquals(7, nonShuffledCount[1], 3); // 4 cards, so expect 40/6 = 7
    }
}
