...
public class CardRecycling {
    ...
    public void recycleOptionsPresentedToPlayersAtTheStartOfEachBattle() {
        ...
        do {
            ...
            Assert.assertTrue(actions.stream().allMatch(a -> a instanceof RecycleCard));
            for (ToadCard  card : state.getPlayerHand(firstPlayer)) {
                Assert.assertTrue(actions.contains( new RecycleCard(card)));
            }
            Assert.assertTrue(actions.contains(new RecycleCard(null)));
            ...
            if (state.getRoundCounter() == 1 && state.getTurnCounter() == 0) {
                ...
                if (state.getBattlesWon(0,0) - state.getBattlesWon(0,1) > 0) {
                    firstPlayer = 0;
                } else {
                    firstPlayer = 1;
                }
    ...
    public void recyclingACardGivesVisibilityOfBottomOfDeck() {
        ...
        Assert.assertFalse(state.getPlayerHand(0).contains(card));
        ...
        for (int i = 0; i < state.getPlayerDeck(0).getSize(); i++) {
            if (i == state.getPlayerDeck(0).getSize() - 1) {
                Assert.assertTrue(state.getPlayerDeck(0).isComponentVisible(i, 0));
            } else {
                Assert.assertFalse(state.getPlayerDeck(0).isComponentVisible(i, 0));
            }
            Assert.assertFalse(state.getPlayerDeck(0).isComponentVisible(i, 1));
        }
    }
}
