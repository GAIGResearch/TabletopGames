...
import games.toads.ToadMCTSPlayer;
...
public class TestUndoOpponentFlank {
    ...
    public void basicInstantiation() {
        ...
        Assert.assertNull(state.getHiddenFlankCard(0));
        assertEquals(3, state.getPlayerHand(0).getSize());
        Assert.assertTrue(state.getPlayerHand(0).contains(flankCard));
        ...
        for (AbstractAction action : actions) {
            Assert.assertTrue(action instanceof PlayFlankCard);
            Assert.assertTrue(p0Hand.contains(((PlayFlankCard) action).card));
        }

        fm.next(state, actions.get(2));
        Assert.assertTrue(state.getActionsInProgress().isEmpty());
        assertEquals(1, state.getCurrentPlayer());
        Assert.assertNotNull(state.getHiddenFlankCard(0));
    }
    ...
    public void oneTreeWorks() {
        ...
        Assert.assertTrue(actionChosen instanceof PlayFieldCard);
        Assert.assertTrue(player.functionalityApplies);
        assertEquals(3, state.getHistory().size());
        Assert.assertEquals(0, (int) state.getHistory().get(2).a);
        Assert.assertTrue(state.getHistory().get(2).b instanceof PlayFlankCard);
        assertEquals(1, state.getCurrentPlayer());
    }
    ...
    public void oneTreeWorksWithReuse() {
        ...
        Assert.assertTrue(dummyAction instanceof PlayFlankCard);
        ...
        actions = fm._computeAvailableActions(state);
        actionChosen = player.getAction(state, actions);
        Assert.assertSame(expectedNewRoot, player.getRoot());
    }
    ...
    public void selfOnlySkipsTheWholePalaver() {
        ...
        AbstractAction actionChosen = player.getAction(state, fm._computeAvailableActions(state));
        Assert.assertTrue(actionChosen instanceof PlayFieldCard);
        Assert.assertFalse(player.functionalityApplies);
        assertEquals(2, state.getHistory().size());
        Assert.assertEquals(0, (int) state.getHistory().get(1).a);
        Assert.assertTrue(state.getHistory().get(1).b instanceof PlayFlankCard);
        assertEquals(1, state.getCurrentPlayer());
    }
...
//    }
    ...
    public void MCGSShufflesCorrectly() {
        ...
        Assert.assertEquals(1, levelOneNodes); // there should be no change in IS at all from the opponent's 'move'
    }
