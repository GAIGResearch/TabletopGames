...
import games.toads.actions.ShowCards;
...
public class Tactics {
    ...
    private void checkVisibilityOfNumberOfCards(int n, int player) {
        ...
        Assert.assertEquals(n, visibleCount);
    }
    ...
    public void iconBearerActivatesScout() {
        ...
        Assert.assertTrue(fm.computeAvailableActions(state).stream().allMatch(a -> a instanceof ShowCards));
    ...
    public void scoutIII() {
        ...
        for (ToadCard card : state.playerHands.get(1).getComponents()) {
            Assert.assertTrue(fm.computeAvailableActions(state).contains(new ShowCards(card.type)));
        }
        ...
        for (int i = 0; i < state.playerHands.get(1).getSize(); i++) {
            ...
            if (card.type == action.cardNotRevealed) {
                Assert.assertFalse(state.playerHands.get(1).isComponentVisible(i, 0));
            } else {
                Assert.assertTrue(state.playerHands.get(1).isComponentVisible(i, 0));
            }
        }
        assertEquals(1, state.getCurrentPlayer());
    ...
    public void assaultCannonNamesCard() {
        ...
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof ForceOpponentDiscard));
        fm.next(state, actions.get(0));
        assertEquals(1, state.getCurrentPlayer());
        actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof PlayFieldCard));
    }
    ...
    public void assaultCannonExcludesDiscardsAndTiebreaker() {
        ...
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof ForceOpponentDiscard));
        Assert.assertFalse(actions.contains(new ForceOpponentDiscard(BERSERKER)));
        Assert.assertFalse(actions.contains(new ForceOpponentDiscard(GENERAL_ONE)));
        Assert.assertTrue(actions.contains(new ForceOpponentDiscard(ToadConstants.ToadCardType.NONE_OF_THESE)));
        fm.next(state, actions.get(0));
        assertEquals(1, state.getCurrentPlayer());
        actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof PlayFieldCard));
    ...
    public void assaultCannonActivatedByIconBearer() {
        ...
        assertEquals(POST_BATTLE, state.getGamePhase());
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertEquals(10, actions.size());  // all possible cards, and NONE_OF_THESE
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof ForceOpponentDiscard));
        fm.next(state, actions.get(0));
        assertEquals(1, state.getCurrentPlayer());
        actions = fm.computeAvailableActions(state);
    ...
    public void assaultCannonShiftIsToBottomAndVisible() {
        ...
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.contains(new ForceOpponentDiscard(card.type)));
        ...
        Assert.assertTrue(state.getPlayerDeck(1).isComponentVisible(2, 0));
        Assert.assertTrue(state.getPlayerDeck(1).isComponentVisible(2, 1));
        Assert.assertFalse(state.getPlayerDeck(1).isComponentVisible(0, 0));
        Assert.assertFalse(state.getPlayerDeck(1).isComponentVisible(0, 1));
        ...
        Assert.assertTrue(copy.getPlayerDeck(1).isComponentVisible(2, 0));
        Assert.assertTrue(copy.getPlayerDeck(1).isComponentVisible(2, 1));
        Assert.assertFalse(copy.getPlayerDeck(1).isComponentVisible(0, 0));
        Assert.assertFalse(copy.getPlayerDeck(1).isComponentVisible(0, 1));
    }
