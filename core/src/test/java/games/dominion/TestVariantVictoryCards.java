...
public class TestVariantVictoryCards {
    ...
    public void oneGarden() {
        ...
        for (int addedCards = 0; addedCards < 33; addedCards++) {
            for (int p = 0; p < 4; p++) {
                Assert.assertEquals(startScore + (p == 1 ? 1 + (addedCards / 10) : 0), (int) state.getGameScore(p));
            }
    ...
    public void twoGardens() {
        ...
        for (int addedCards = 0; addedCards < 33; addedCards++) {
            for (int p = 0; p < 4; p++) {
                Assert.assertEquals(startScore + (p == 1 ? 2 + (addedCards / 10) * 2 : 0), (int) state.getGameScore(p));
            }
            state.addCard(CardType.COPPER, 1, DeckType.DISCARD);
        }
    }
