...
public class TestPartialObservabilityCopy {
    ...
    public void handsOfOtherPlayersAreChanged() {
        DominionGameState myCopy = (DominionGameState) state.copy(0);
        DominionGameState fullCopy = (DominionGameState) state.copy();
        Assert.assertFalse(myCopy.getDeck(DeckType.HAND, 1).equals(state.getDeck(DeckType.HAND, 1)));
        Assert.assertFalse(myCopy.getDeck(DeckType.HAND, 2).equals(state.getDeck(DeckType.HAND, 2)));
        Assert.assertFalse(myCopy.getDeck(DeckType.HAND, 3).equals(state.getDeck(DeckType.HAND, 3)));
        ...
        Assert.assertTrue(partCopyHand.getDeckVisibility()[1]);
        IntStream.of(0, 2, 3).forEach(i -> Assert.assertFalse(partCopyHand.getDeckVisibility()[i]));
    }
    ...
    public void ownDrawPileIsShuffled() {
        DominionGameState myCopy = (DominionGameState) state.copy(0);
        DominionGameState fullCopy = (DominionGameState) state.copy();
        Assert.assertFalse(myCopy.getDeck(DeckType.DRAW, 0).equals(state.getDeck(DeckType.DRAW, 0)));
        ...
        Assert.assertEquals(originalCards, myCopyCards);
    }
    ...
    public void otherPlayersHandsAndDrawPilesAreShuffled() {
        ...
        for (int playerId = 1; playerId < 4; playerId++) {
            Assert.assertFalse(myCopy.getDeck(DeckType.DRAW, playerId).equals(state.getDeck(DeckType.DRAW, playerId)));
    ...
    private void checkVisibilityExpectations(PartialObservableDeck<DominionCard> playerHand) {
        for (int p = 0; p < 4; p++) {
            Assert.assertTrue(playerHand.getVisibilityForPlayer(0, p));
            for (int j = 1; j < 4; j++)
                if (p != 0)
                    Assert.assertFalse(playerHand.getVisibilityForPlayer(j, p));
        }
