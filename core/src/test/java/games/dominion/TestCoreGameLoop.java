...
public class TestCoreGameLoop {
    ...
    public void initialHandDealIsCorrect() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            Deck<DominionCard> allCards = new Deck<>("", CoreConstants.VisibilityMode.HIDDEN_TO_ALL);
    ...
    public void purchaseOptionsAreCorrect() {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            switch (i) {
                case 9:
                case 8:
                    Assert.assertTrue(availableCards.contains(CardType.PROVINCE));
                case 7:
                case 6:
                    Assert.assertTrue(availableCards.contains(CardType.GOLD));
                case 5:
                    Assert.assertTrue(availableCards.contains(CardType.DUCHY));
                case 4:
                case 3:
                    Assert.assertTrue(availableCards.contains(CardType.SILVER));
                case 2:
                    Assert.assertTrue(availableCards.contains(CardType.ESTATE));
                case 1:
                case 0:
                    Assert.assertTrue(availableCards.contains(CardType.COPPER));
                    Assert.assertTrue(fm.computeAvailableActions(state).contains(new EndPhase(DominionGamePhase.Buy)));
            }
        }
    }
    ...
    public void canBuyMoreThanOneCard() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            List<AbstractAction> actions = fm.computeAvailableActions(state);
            Assert.assertTrue(actions.contains(new EndPhase(DominionGamePhase.Buy)));
            Assert.assertTrue(actions.contains(newBuy));
            fm.computeAvailableActions(state);
            fm.next(state, newBuy);
        }
        assertEquals(1, state.buysLeft());
    ...
    public void provinceGameEndCondition() {
        DominionGameState state = (DominionGameState) game.getGameState();
        for (int i = 0; i < 12; i++) {
            Assert.assertFalse(state.gameOver());
    ...
    public void canPlayACardFromHand() {
        ...
        Assert.assertTrue(actions.contains(new EndPhase(DominionGamePhase.Play)));
        state.addCard(CardType.VILLAGE, 0, DeckType.HAND);
        actions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, actions.size());
        Assert.assertTrue(actions.contains(new EndPhase(DominionGamePhase.Play)));
        Assert.assertTrue(actions.contains(new SimpleAction(CardType.VILLAGE, 0)));
        state.addCard(CardType.SMITHY, 0, DeckType.HAND);
        actions = fm.computeAvailableActions(state);
        Assert.assertEquals(3, actions.size());
        Assert.assertTrue(actions.contains(new EndPhase(DominionGamePhase.Play)));
        Assert.assertTrue(actions.contains(new SimpleAction(CardType.VILLAGE, 0)));
        Assert.assertTrue(actions.contains(new SimpleAction(CardType.SMITHY, 0)));
    }
    ...
    public void cannotPlayAnActionCardDuringBuyPhase() {
        ...
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.contains(new EndPhase(DominionGamePhase.Buy)));
        Assert.assertFalse(actions.contains(new SimpleAction(CardType.VILLAGE, 0)));
    }
