...
public class BaseActionCardsTest {
    ...
    public void cellarBase() {
        ...
        Assert.assertEquals(3, cellarActions.size());
        Assert.assertTrue(cellarActions.contains(new DiscardCard(CardType.ESTATE, 0)));
        Assert.assertTrue(cellarActions.contains(new DiscardCard(CardType.COPPER, 0)));
        Assert.assertTrue(cellarActions.contains(new DoNothing()));
    }
    ...
    public void cellarDiscardsAndDraws() {
        ...
        Assert.assertNull(state.currentActionInProgress());

        List<AbstractAction> nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(1, nextActions.size());
    ...
    public void militiaCausesAllOtherPlayersToDiscardDownToFive() {
        ...
        do {
            ...
            Assert.assertTrue(actionsAvailable.stream().allMatch(a -> a instanceof DiscardCard));
    ...
    public void moat() {
        ...
        assertEquals(1, state.getDeck(DeckType.TABLE, 0).getSize());
        assertEquals(0, state.actionsLeft());
        Assert.assertFalse(state.isDefended(0));
    }
    ...
    public void moatDefendsAgainstMilitia() {
        ...
        assertEquals(0, state.getCurrentPlayer());
        List<AbstractAction> actionsAvailable = fm.computeAvailableActions(state);
        Assert.assertEquals(2, actionsAvailable.size());
        Assert.assertTrue(actionsAvailable.contains(new DoNothing()));
        Assert.assertTrue(actionsAvailable.contains(new MoatReaction(0)));

        Assert.assertFalse(state.isDefended(0));
        fm.next(state, new MoatReaction(0));
        Assert.assertTrue(state.isDefended(0));
        ...
        for (int i = 1; i < 3; i++) {
            Assert.assertFalse(state.isDefended(i));
            Assert.assertTrue(hand.getVisibilityForPlayer(0, i));
        }
    ...
    public void notRevealingMoatDoesNotDefendAgainstMilitia() {
        ...
        for (int i = 0; i < 3; i++) {
            Assert.assertFalse(state.isDefended(i));
            if (i != 0)
                Assert.assertFalse(hand.getVisibilityForPlayer(0, i));
        }
        assertEquals(0, state.getCurrentPlayer());
    ...
    public void moatDefenceStatusEndsWithTurn() {
        DominionGameState state = (DominionGameState) game.getGameState();
        state.setDefended(3);
        Assert.assertTrue(state.isDefended(3));
        moveForwardToNextPlayer(state);
        for (int i = 0; i < 4; i++) {
            Assert.assertFalse(state.isDefended(i));
        }
    ...
    public void allPlayersDefendingAgainstMilitiaMovesProgressOn() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            Assert.assertTrue(state.isDefended(i + 1));
        }
        assertEquals(0, state.getCurrentPlayer());
        assertEquals(DominionGamePhase.Buy, state.getGamePhase());
    }
    ...
    public void remodelForcesATrash() {
        ...
        assertEquals(0, state.getCurrentPlayer());
        assertEquals(Play, state.getGamePhase());
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof TrashCard));
        Assert.assertEquals(3, actions.size()); // COPPER, GOLD, ESTATE
    ...
    public void remodelWithNoCardsInHand() {
        ...
        assertEquals(0, state.getCurrentPlayer());
        assertEquals(DominionGamePhase.Buy, state.getGamePhase());
        Assert.assertFalse(state.isActionInProgress());
    }
    ...
    public void remodelBuyOptionsCorrectGivenTrashedCard() {
        ...
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof GainCard));
        Assert.assertTrue(actions.stream().allMatch(a -> ((GainCard) a).cardType.cost <= 4));
        ...
        Assert.assertTrue(allCards.stream().allMatch(c -> c.cost >= 5));
    ...
    public void remodelPossibleWithNoBuyableCards() {
        ...
        assertEquals(Play, state.getGamePhase());
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof TrashCard));
        Assert.assertEquals(2, actions.size()); // COPPER, ESTATE
        ...
        Assert.assertTrue(state.isActionInProgress());
        fm.next(state, new DoNothing());
        Assert.assertFalse(state.isActionInProgress());
    ...
    public void workshop() {
        ...
        assertEquals(0, state.actionsLeft());
        assertEquals(0, state.getCurrentPlayer());
        Assert.assertFalse(workshop.executionComplete(state));
        List<AbstractAction> availableActions = fm.computeAvailableActions(state);
        Assert.assertTrue(availableActions.stream().allMatch(a -> a instanceof GainCard));
        Assert.assertEquals(11, availableActions.size()); // COPPER, SILVER, ESTATE, CELLAR, MOAT, MERCHANT, VILLAGE, WORKSHOP, MILITIA, REMODEL, SMITHY
        availableActions.forEach(a -> {
                    GainCard gc = (GainCard) a;
                    Assert.assertTrue(gc.cardType.cost <= 4);
        ...
        assertEquals(0, state.actionsLeft());
        assertEquals(0, state.getCurrentPlayer());
        Assert.assertTrue(workshop.executionComplete(state));
    ...
    public void mine() {
        ...
        assertEquals(0, state.actionsLeft());
        assertEquals(0, state.getCurrentPlayer());
        Assert.assertFalse(mine.executionComplete(state));
        assertEquals(Play, state.getGamePhase());

        List<AbstractAction> availableActions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, availableActions.size());
        Assert.assertTrue(availableActions.contains(new TrashCard(CardType.COPPER, 0)));
        Assert.assertTrue(availableActions.contains(new TrashCard(CardType.SILVER, 0)));

        fm.next(state, new TrashCard(CardType.SILVER, 0));
        Assert.assertFalse(mine.executionComplete(state));
        assertEquals(Play, state.getGamePhase());
        availableActions = fm.computeAvailableActions(state);
        Assert.assertEquals(3, availableActions.size());
        Assert.assertTrue(availableActions.contains(new GainCard(CardType.COPPER, 0, DeckType.HAND)));
        Assert.assertTrue(availableActions.contains(new GainCard(CardType.SILVER, 0, DeckType.HAND)));
        Assert.assertTrue(availableActions.contains(new GainCard(CardType.GOLD, 0, DeckType.HAND)));

        fm.next(state, new GainCard(CardType.GOLD, 0, DeckType.HAND));
        assertEquals(startSpend + 1, state.availableSpend(0));
        Assert.assertTrue(mine.executionComplete(state));
        assertEquals(DominionGamePhase.Buy, state.getGamePhase());
    }
    ...
    public void artisan() {
        ...
        Assert.assertTrue(availableActions.stream().allMatch(a -> a instanceof GainCard));
        Assert.assertTrue(availableActions.stream().allMatch(a -> ((GainCard) a).cardType.cost <= 5));
        Assert.assertTrue(availableActions.contains(new GainCard(CardType.MINE, 0, DeckType.HAND)));
        Assert.assertFalse(artisan.executionComplete(state));

        fm.next(state, new GainCard(CardType.MINE, 0, DeckType.HAND));
        availableActions = fm.computeAvailableActions(state);

        Assert.assertTrue(availableActions.stream().allMatch(a -> a instanceof MoveCard));
        Assert.assertEquals(4, availableActions.size());
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.MINE, 0, DeckType.HAND, 0, DeckType.DRAW, false)));
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.COPPER, 0, DeckType.HAND, 0, DeckType.DRAW, false)));
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.ESTATE, 0, DeckType.HAND, 0, DeckType.DRAW, false)));
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.VILLAGE, 0, DeckType.HAND, 0, DeckType.DRAW, false)));
        Assert.assertFalse(artisan.executionComplete(state));

        fm.next(state, new MoveCard(CardType.MINE, 0, DeckType.HAND, 0, DeckType.DRAW, false));
        Assert.assertTrue(artisan.executionComplete(state));
        assertEquals(DominionGamePhase.Buy, state.getGamePhase());

        PartialObservableDeck<DominionCard> drawDeck = (PartialObservableDeck<DominionCard>) state.getDeck(DeckType.DRAW, 0);
        Assert.assertTrue(drawDeck.getVisibilityOfComponent(0)[0]); // player 0 can see the card; no-one else can
        for (int i = 1; i < 4; i++) Assert.assertFalse(drawDeck.getVisibilityOfComponent(0)[i]);
        for (int i = 0; i < 4; i++) Assert.assertFalse(drawDeck.getVisibilityOfComponent(1)[i]);
        ...
        Assert.assertTrue(copyDrawDeck.getVisibilityOfComponent(0)[0]); // player 0 can see the card; no-one else can
        for (int i = 1; i < 4; i++) Assert.assertFalse(copyDrawDeck.getVisibilityOfComponent(0)[i]);
        for (int i = 0; i < 4; i++) Assert.assertFalse(copyDrawDeck.getVisibilityOfComponent(1)[i]);
    ...
    public void poacherWithTwoEmptyPiles() {
        ...
        List<AbstractAction> availableActions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, availableActions.size());
        Assert.assertTrue(availableActions.contains(new DiscardCard(CardType.COPPER, 0)));
        Assert.assertTrue(availableActions.contains(new DiscardCard(CardType.ESTATE, 0)));

        fm.next(state, new DiscardCard(CardType.COPPER, 0));
        Assert.assertFalse(poacher.executionComplete(state));
        assertEquals(Play, state.getGamePhase());
        fm.next(state, new DiscardCard(CardType.ESTATE, 0));
        int finalSpend = state.getDeck(DeckType.HAND, 0).sumInt(DominionCard::treasureValue);
        Assert.assertTrue(poacher.executionComplete(state));
        assertEquals(Play, state.getGamePhase());
    ...
    public void witch() {
        ...
        Assert.assertTrue(fm.computeAvailableActions(state).contains(witch));
    ...
    public void witchWithAMoatAndOneCurse() {
        ...
        fm.next(state, witch);
        assertEquals(1, state.cardsOfType(CardType.CURSE, -1, DeckType.SUPPLY));
        Assert.assertTrue(fm.computeAvailableActions(state).contains(new MoatReaction(1)));
    ...
    public void chapel() {
        ...
        assertEquals(3, state.getDeck(DeckType.HAND, 0).getSize());
        assertEquals(5, state.getDeck(DeckType.DRAW, 0).getSize());
        Assert.assertFalse(chapel.executionComplete(state));
        assertEquals(chapel, state.currentActionInProgress());

        fm.next(state, new DoNothing());
        Assert.assertTrue(chapel.executionComplete(state));
        assertEquals(0, state.getDeck(DeckType.DISCARD, 0).getSize());
        assertEquals(3, state.getDeck(DeckType.HAND, 0).getSize());
        assertEquals(5, state.getDeck(DeckType.DRAW, 0).getSize());
        Assert.assertNull(state.currentActionInProgress());
    ...
    public void harbingerWithDiscard() {
        ...
        Assert.assertFalse(harbinger.executionComplete(state));
        assertEquals(4, state.getDeck(DeckType.DRAW, 0).getSize());
        assertEquals(harbinger, state.currentActionInProgress());
        List<AbstractAction> availableActions = fm.computeAvailableActions(state);

        Assert.assertEquals(3, availableActions.size());
        Assert.assertTrue(availableActions.contains(new DoNothing()));
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.HARBINGER, 0, DeckType.DISCARD, 0, DeckType.DRAW, false)));
        Assert.assertTrue(availableActions.contains(new MoveCard(CardType.SILVER, 0, DeckType.DISCARD, 0, DeckType.DRAW, false)));

        fm.next(state, new MoveCard(CardType.SILVER, 0, DeckType.DISCARD, 0, DeckType.DRAW, false));
        Assert.assertTrue(harbinger.executionComplete(state));
        Assert.assertNull(state.currentActionInProgress());
        ...
        PartialObservableDeck<DominionCard> drawDeck = (PartialObservableDeck<DominionCard>) state.getDeck(DeckType.DRAW, 0);
        Assert.assertTrue(drawDeck.getVisibilityForPlayer(0, 0));
        for (int i = 1; i < 4; i++)
            Assert.assertFalse(drawDeck.getVisibilityForPlayer(0, i));
    ...
    public void throneRoomWithMarket() {
        ...
        assertNotSame(DominionCard.create(CardType.MARKET).getAction(0, false), nextActions.get(0));
    ...
    public void throneRoomWithWorkshop() {
        ...
        nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(11, nextActions.size()); // COPPER, SILVER, ESTATE, CELLAR, MOAT, MERCHANT, VILLAGE, WORKSHOP, MILITIA, REMODEL, SMITHY
        ...
        nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(11, nextActions.size());
    ...
    public void throneRoomWithThroneRoomWithSingleMarket() {
        ...
        List<AbstractAction> nextActions = fm.computeAvailableActions(state);
        assertEquals(throneRoom, state.currentActionInProgress());
        Assert.assertSame(throneRoom, state.currentActionInProgress());
        Assert.assertEquals(2, nextActions.size());
        ...
        fm.next(state, nextActions.get(0));
        assertEquals(1, state.actionsLeft());
        Assert.assertNotSame(throneRoom, state.currentActionInProgress());
        Assert.assertTrue(state.currentActionInProgress() instanceof ThroneRoom);
    ...
    public void bandit() {
        ...
        List<AbstractAction> nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, nextActions.size());
        Assert.assertTrue(nextActions.contains(new TrashCard(CardType.SILVER, 1, DeckType.DISCARD)));
        Assert.assertTrue(nextActions.contains(new TrashCard(CardType.GOLD, 1, DeckType.DISCARD)));
        ...
        nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, nextActions.size());
        Assert.assertTrue(nextActions.contains(new DoNothing()));
        Assert.assertTrue(nextActions.contains(new MoatReaction(3)));
    ...
    public void bureaucrat() {
        ...
        nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, nextActions.size());
        Assert.assertTrue(nextActions.contains(new MoveCard(CardType.ESTATE, 3, DeckType.HAND, 3, DeckType.DRAW, true)));
        Assert.assertTrue(nextActions.contains(new MoveCard(CardType.DUCHY, 3, DeckType.HAND, 3, DeckType.DRAW, true)));
        fm.next(state, nextActions.get(0));
        ...
        for (int i = 0; i < state.getDeck(DeckType.HAND, 2).getSize(); i++)
            for (int j = 0; j < state.getNPlayers(); j++)
                Assert.assertTrue(((PartialObservableDeck<DominionCard>) state.getDeck(DeckType.HAND, 2)).getVisibilityForPlayer(i, j));
    ...
    public void sentryWithNoneKept() {
        ...
        assertEquals(2, state.getDeck(DeckType.DRAW, 0).getSize());
        List<AbstractAction> nextActions = fm.computeAvailableActions(state);
        Assert.assertEquals(3, nextActions.size());
    ...
    public void sentryWithBothKept() {
        ...
        Assert.assertNotSame(CardType.SILVER, secondCard);
        fm.next(state, nextActions.get(2));
        nextActions = fm.computeAvailableActions(state);
        fm.next(state, nextActions.get(2));
