        ...
        public void onEvent(Event event) {
            if (event.type == ROUND_OVER) {
                ...
                for (int i = 0; i < state.getTurnOrder().getRoundCounter(); i++)
                    Assert.assertTrue(state.getRounds().getVisibilityForPlayer(i, 0));
                ...
                for (int loop = 0; loop < 30; loop++) {
                    ...
                    for (int i = 0; i < copyState.getRounds().getSize(); i++) {
                        ...
                        if (state.getRounds().isComponentVisible(i, 0)) {
                            ...
                            Assert.assertFalse(visibleCards.contains(copyState.getRounds().get(i).getComponentName()));
                        }
                    }
                }
                ...
                if (visibleRoundCards < 5) {// on the last round we know everything
                    if (visibleRoundCards == 4) {
                        ...
                        Assert.assertEquals((double) matches / (matches + nonMatches), 0.333, 0.333);
                        Assert.assertTrue(matches > 0);
                    } else {
                        Assert.assertTrue(nonMatches > matches);
                        Assert.assertTrue(matches > 0); // we should have something match by chance!
                    }
                }
            }
