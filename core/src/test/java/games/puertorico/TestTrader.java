...
public class TestTrader {
    ...
    public void playersReceiveAppropriateMoniesForSale() {
        ...
        for (PuertoRicoConstants.Crop c : PuertoRicoConstants.Crop.values()) {
            Assert.assertFalse(state.getMarket().contains(c));
        }
        ...
        Assert.assertTrue(state.getMarket().contains(CORN));
        ...
        Assert.assertFalse(state.getMarket().contains(SUGAR));
        Assert.assertTrue(state.getMarket().isEmpty());
    ...
    public void playersWithNothingToTradeAreAutoSkipped() {
        ...
        Assert.assertTrue(state.getMarket().contains(COFFEE));
        Assert.assertTrue(state.getMarket().contains(SUGAR));
    ...
    public void onePlayerCannotSellTwoDifferentGoods() {
        ...
        assertEquals(1, state.getCurrentPlayer());
        Assert.assertFalse(state.isActionInProgress());  // we have moved on to SelectRole
        Assert.assertTrue(fm.computeAvailableActions(state).stream().allMatch(a -> a instanceof SelectRole));
    }
