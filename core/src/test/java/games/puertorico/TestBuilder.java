...
public class TestBuilder {
    ...
    public void builderAllowsOptionsUpToBudget() {
        ...
        // we start with
        Assert.assertTrue(actions.contains(new DoNothing()));
        Assert.assertTrue(actions.contains(new Build(SMALL_INDIGO_PLANT)));
        Assert.assertFalse(actions.contains(new Build(COFFEE_ROASTER)));
        List<Build> builds = actions.stream()
                .filter(a -> a instanceof Build)
                .map(a -> (Build) a).collect(Collectors.toList());
        Assert.assertTrue(builds.stream().allMatch(b -> b.type.cost <= money));
        Assert.assertTrue(state.getAvailableBuildings().stream()
    ...
    public void roleOwnerGetsDiscountOfOne() {
        ...
        // we start with
        Assert.assertTrue(actions.contains(new DoNothing()));
        Assert.assertTrue(actions.contains(new Build(SMALL_INDIGO_PLANT, 0)));
        Assert.assertFalse(actions.contains(new Build(COFFEE_ROASTER, 5)));
        List<Build> builds = actions.stream()
                .filter(a -> a instanceof Build)
                .map(a -> (Build) a).collect(Collectors.toList());
        Assert.assertTrue(builds.stream().allMatch(b -> b.type.cost <= money + 1 && b.cost <= money));
        Assert.assertTrue(state.getAvailableBuildings().stream()
    ...
    public void buildingExhaustionRemovesFromOptions() {
        ...
        Assert.assertFalse(actions.stream().anyMatch(a -> a instanceof Build && ((Build) a).type == INDIGO_PLANT));
    }
    ...
    public void cannotBuildBeyondAvailableTownSlots() {
        ...
        Assert.assertTrue(actions.contains(new Build(PuertoRicoConstants.BuildingType.INDIGO_PLANT)));
        Assert.assertFalse(actions.contains(new Build(FORTRESS)));
        Assert.assertTrue(actions.stream().filter(a -> a instanceof Build).map(a -> (Build) a).allMatch(b -> b.type.size == 1));
    }
    ...
    public void cannotBuildMoreThanFourLargeBuildings() {
        ...
        Assert.assertTrue(actions.contains(new Build(PuertoRicoConstants.BuildingType.INDIGO_PLANT)));
        Assert.assertFalse(actions.contains(new Build(CITY_HALL)));
        Assert.assertTrue(actions.stream().filter(a -> a instanceof Build).map(a -> (Build) a).allMatch(b -> b.type.size == 1));
    }
    ...
    public void quarryProvidesDiscount() {
        ...
        Assert.assertTrue(actions.contains(new Build(SMALL_INDIGO_PLANT, 0)));
        Assert.assertTrue(actions.contains(new Build(SMALL_SUGAR_MILL, 0)));
        Assert.assertTrue(actions.contains(new Build(SUGAR_MILL, 2)));
        Assert.assertFalse(actions.contains(new Build(COFFEE_ROASTER, 4)));
        ...
        Assert.assertTrue(actions.contains(new Build(SMALL_INDIGO_PLANT, 0)));
        Assert.assertTrue(actions.contains(new Build(SMALL_SUGAR_MILL, 0)));
        Assert.assertTrue(actions.contains(new Build(SUGAR_MILL, 1)));
        Assert.assertFalse(actions.contains(new Build(COFFEE_ROASTER, 3)));
        ...
        Assert.assertTrue(actions.contains(new Build(SMALL_SUGAR_MILL, 0)));
        Assert.assertTrue(actions.contains(new Build(SUGAR_MILL, 1)));
        Assert.assertTrue(actions.contains(new Build(COFFEE_ROASTER, 2)));
    }
    ...
    public void inactiveUniversityDoesNotAddColonistToBuilding() {
        ...
        Assert.assertFalse(state.hasActiveBuilding(1, build.type));
    ...
    public void activeUniversityDoesAddColonistToBuilding() {
        ...
        Build build = (Build) fm.computeAvailableActions(state).get(0);
        fm.next(state, build); // build
        Assert.assertTrue(state.hasActiveBuilding(1, build.type));
