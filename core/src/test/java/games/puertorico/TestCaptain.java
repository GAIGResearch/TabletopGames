...
public class TestCaptain {
    ...
    public void testCaptainHasOptionOfAvailableShips() {
        ...
        Assert.assertTrue(actions.contains(new ShipCargo(INDIGO, 0, 3)));
        Assert.assertTrue(actions.contains(new ShipCargo(INDIGO, 1, 3)));
        Assert.assertTrue(actions.contains(new ShipCargo(INDIGO, 2, 3)));
        ...
        Assert.assertEquals(3, actions.size());
        Assert.assertTrue(actions.contains(new ShipCargo(INDIGO, 0, 1)));
        Assert.assertTrue(actions.contains(new ShipCargo(CORN, 1, 1)));
        Assert.assertTrue(actions.contains(new ShipCargo(CORN, 2, 1)));
    ...
    public void testAllPlayersMustSellEverythingIfPossible() {
        ...
        Assert.assertTrue(fm.computeAvailableActions(state).stream().allMatch(a -> a instanceof ShipCargo));
        fm.next(state, fm.computeAvailableActions(state).get(0));  // ships INDIGO/CORN
    ...
    public void testCaptainGainsExtraVPOnTheirLastShipmentOnly() {
        ...
        fm.next(state, new SelectRole(PuertoRicoConstants.Role.CAPTAIN));
        Assert.assertTrue(fm.computeAvailableActions(state).contains(new ShipCargo(TOBACCO, 0, 2)));
        ...
        Assert.assertTrue(fm.computeAvailableActions(state).contains(new ShipCargo(INDIGO, 2, 2)));
        ...
        while (state.isActionInProgress()) {
            ...
            if (!(nextActions.get(0) instanceof DoNothing)) {
                ...
                if (state.getCurrentPlayer() == 0) {
                    ...
                    Assert.assertTrue(nextActions.contains(new DiscardGoodsExcept(TOBACCO)));
                } else {
                    Assert.assertEquals(3, nextActions.size());
                    assertEquals(2, state.getCurrentPlayer());
                    Assert.assertTrue(state.isActionInProgress());
                    Assert.assertTrue(nextActions.contains(new DiscardGoodsExcept(TOBACCO)));
                    Assert.assertTrue(nextActions.contains(new DiscardGoodsExcept(COFFEE)));
                    Assert.assertTrue(nextActions.contains(new DiscardGoodsExcept(SUGAR)));
                    discardOptionFound = true;
                }
            }
            fm.next(state, fm.computeAvailableActions(state).get(0));
        }
        Assert.assertTrue(discardOptionFound);
    ...
    public void smallWarehouseAutomaticallyKeepsGoods() {
        ...
        Assert.assertTrue(actions.stream().allMatch(a -> a instanceof WarehouseStorage));
        fm.next(state, fm.computeAvailableActions(state).get(0)); // either is fine; and we should not be prompted again

        assertEquals(2, state.getCurrentPlayer());
        Assert.assertTrue(state.isActionInProgress());
        Assert.assertTrue(state.getActionsInProgress().peek() instanceof DiscardPhase);
    ...
    public void smallWarehouseAsksForDecisionOnGoodToBeStored() {
        ...
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertEquals(3, actions.size());
        Assert.assertTrue(actions.contains(new WarehouseStorage(TOBACCO)));
        Assert.assertTrue(actions.contains(new WarehouseStorage(COFFEE)));
        Assert.assertTrue(actions.contains(new WarehouseStorage(SUGAR)));
        fm.next(state, new WarehouseStorage(TOBACCO));
        assertEquals(0, state.getCurrentPlayer());
        actions = fm.computeAvailableActions(state);
        Assert.assertEquals(2, actions.size());
        Assert.assertTrue(actions.contains(new DiscardGoodsExcept(SUGAR)));
        Assert.assertTrue(actions.contains(new DiscardGoodsExcept(COFFEE)));
    ...
    public void warehousesClearedAfterCaptainAndDiscardPhase() {
        ...
        Assert.assertTrue(state.getPlayerBoard(0).getCropsInWarehouses().contains(COFFEE));
    ...
    public void wharfProvidesAnAdditionalActionOncePublicShipActionAreExhausted() {
        ...
        Assert.assertTrue(actions.stream().anyMatch(a -> a instanceof ShipCargo && ((ShipCargo) a).shipNumber == 12));
        ...
        actions = fm.computeAvailableActions(state);
        Assert.assertTrue(actions.stream().anyMatch(a -> a instanceof ShipCargo && ((ShipCargo) a).shipNumber == 12));
        Assert.assertTrue(actions.stream().noneMatch(a -> a instanceof ShipCargo && ((ShipCargo) a).shipNumber < 10));
        ...
        Assert.assertTrue(state.hasActiveBuilding(2, WHARF));
    }
    ...
    public void wharfCanTakeGoodAlreadyLoadedOnAPublicShip() {
        ...
        List<AbstractAction> actions = fm.computeAvailableActions(state);
        Assert.assertEquals(1, actions.size());
        Assert.assertTrue(actions.contains(new ShipCargo( CORN, 12, 3)));
