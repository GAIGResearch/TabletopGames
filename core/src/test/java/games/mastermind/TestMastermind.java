...
public class TestMastermind {
    ...
    public void testResultGeneratedAfterGuessingEntireRow() {
        ...
        for (int i=0 ; i<mmp.boardWidth ; i++) {
            ...
            if (i != mmp.boardWidth-1) {
                assertEquals(mmgs.getResultBoard().getElement(0,0).getTokenType(), MMConstants.emptyPeg);
                assertEquals(mmgs.getResultBoard().getElement(1,0).getTokenType(), MMConstants.emptyPeg);
                assertEquals(mmgs.getResultBoard().getElement(2,0).getTokenType(), MMConstants.emptyPeg);
                assertEquals(mmgs.getResultBoard().getElement(3,0).getTokenType(), MMConstants.emptyPeg);
            } else {

                assertNotSame(mmgs.getResultBoard().getElement(0,0).getTokenType(), MMConstants.emptyPeg);
                assertNotSame(mmgs.getResultBoard().getElement(1,0).getTokenType(), MMConstants.emptyPeg);
                assertNotSame(mmgs.getResultBoard().getElement(2,0).getTokenType(), MMConstants.emptyPeg);
                assertNotSame(mmgs.getResultBoard().getElement(3,0).getTokenType(), MMConstants.emptyPeg);
            }
        }
    }
    ...
    private boolean _testCorrectResult(String guess, String answerString, String expectedResult) {
        ...
        for (int i=0; i<4; i++) {
            forwardModel.next(mmgs, new SetGridValueAction<>(mmgs.getGuessBoard().getComponentID(), i, 0, MMConstants.guessColours.get(Character.getNumericValue(guess.charAt(i)))));
