...
public class STNRollout extends SingleTreeNode {
    ...
    protected void oneSearchIteration() {
        ...
        Assert.assertEquals(staticRolloutDepth, rolloutActions.size());
        switch (params.rolloutTermination) {
            case DEFAULT:
                // in this case we just check that we have 10 actions in the rollout
                Assert.assertEquals(expectedRolloutLength, staticRolloutDepth);
                break;
            case START_ACTION:
                ...
                Assert.assertTrue(staticRolloutDepth >= expectedRolloutLength);
                assertEquals(0, openLoopState.getTurnOwner());
                assertNotSame(0, rolloutActions.get(rolloutActions.size() - 1).a.intValue());
                break;
            case END_ACTION:
                // in this case we have at least 10 actions, and finish at the end of a player's Turn
                ...
                Assert.assertTrue(staticRolloutDepth >= expectedRolloutLength);
                assertNotSame(openLoopState.getTurnOwner(), rolloutActions.get(rolloutActions.size() - 1).a.intValue());
                assertEquals(0, rolloutActions.get(rolloutActions.size() - 1).a.intValue());
                break;
            case END_TURN:
                Assert.assertTrue(staticRolloutDepth >= expectedRolloutLength);
                assertNotSame(openLoopState.getTurnCounter(), staticStartTurn);
                break;
            case END_ROUND:
                Assert.assertTrue(staticRolloutDepth >= expectedRolloutLength);
                assertNotSame(openLoopState.getRoundCounter(), staticStartRound);
                break;
        }
    }
