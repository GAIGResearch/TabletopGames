        ...
        do {
            ...
            if (!oneAction) {
                ...
                if (oldRoots[currentPlayer] != null) {
                    assertNotSame(oldRoots[currentPlayer], newRoot);
                    if (currentPlayer == 0) {  // the one with reuseTree = true
                        // old root still contains the starting state key
                        Assert.assertTrue(oldRoots[currentPlayer].getTranspositionMap().containsKey(oldOldKeys[currentPlayer]));
                        ...
                        if (!searchExitedGraph) {
                            Assert.assertSame(newRoot, oldRoots[currentPlayer].getTranspositionMap().get(oldKeys[currentPlayer]));
                        }
                        ...
                        if (oldOldOldKeys[currentPlayer] != null) {
                            Assert.assertFalse(newRoot.getTranspositionMap().containsKey(oldOldOldKeys[currentPlayer]));
                        }
                        ...
                        for (Object key : visitMapBeforeAction.keySet()) {
                            if (visitMapAfterAction.containsKey(key)) {
                                Assert.assertTrue(visitMapAfterAction.get(key) >= visitMapBeforeAction.get(key));
