...
public class StageBDecoupledTests {
    ...
    public void rootIsMultiActorWithItsOwnDecisionOwner() {
        ...
        // the root still knows whose search this is; a multi-actor child does not
        assertEquals(0, root.getActor());
        assertEquals(-1, firstChild(root).getActor());
    ...
    @Test
    public void noArgAccessorsThrowAtMultiActorChild() {
        SingleTreeNode child = firstChild(search(p -> {
        }));
        assertEquals(-1, child.getActor());
        assertThrows(AssertionError.class, child::getActionValues);
        assertThrows(AssertionError.class, child::getActionsFromOpenLoopState);
    ...
    @Test
    public void nonOneTreePolicyIsNeverDecoupled() {
        // the node-level gate, for parameters set directly rather than through _reset()
        SingleTreeNode root = search(p -> p.opponentTreePolicy = MCTSEnums.OpponentTreePolicy.SelfOnly);
        assertFalse(root.isMultiActor());
        MCTSParams viaReset = new MCTSParams();
        viaReset.setParameterValue("opponentTreePolicy", MCTSEnums.OpponentTreePolicy.MCGS);
        assertFalse(viaReset.decoupled);
        viaReset.setParameterValue("opponentTreePolicy", MCTSEnums.OpponentTreePolicy.OneTree);
        assertTrue(viaReset.decoupled);
    }
    ...
    static {
        ...
        EXPECTED.put("ucb",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2549|copies=201|rollout=2000|digest=67ed23401c4c7e68|rnd=2228064810716131872");
        EXPECTED.put("ucbTuned",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2586|copies=201|rollout=2000|digest=76d04681b5f43a40|rnd=8663219514887876724");
        EXPECTED.put("exp3",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2538|copies=201|rollout=2000|digest=f367075bf8fed5c4|rnd=745567633726733450");
        EXPECTED.put("regretMatching",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2644|copies=201|rollout=2000|digest=5e0399ef17f3b35f|rnd=3032803597294232314");
        EXPECTED.put("greedy",
                "type=SingleTreeNode|roots=1|visits=200|nodes=196|fm=3010|copies=201|rollout=2000|digest=33542348238036ab|rnd=-7828797891413438531");
        EXPECTED.put("closedLoop",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2000|copies=200|rollout=2000|digest=2719446fd4efc3fd|rnd=-8021293852782020870");
        EXPECTED.put("mast",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2549|copies=201|rollout=2000|digest=67ed23401c4c7e68|rnd=2228064810716131872");
        EXPECTED.put("progressiveWidening",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2544|copies=201|rollout=2000|digest=98ec9e724f8a960c|rnd=-8907240076208972560");
        EXPECTED.put("pUCT",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2633|copies=201|rollout=2000|digest=7fd58c421c1d9cad|rnd=-786632837516347028");
        EXPECTED.put("initialiseVisits",
                "type=SingleTreeNode|roots=1|visits=209|nodes=201|fm=2534|copies=201|rollout=2000|digest=f7ca3907dbc75f25|rnd=-934894581532007506");
        EXPECTED.put("paranoid",
                "type=SingleTreeNode|roots=1|visits=200|nodes=201|fm=2521|copies=201|rollout=2000|digest=50e4d2ce3a5dc631|rnd=-3001675053116265653");
    }
