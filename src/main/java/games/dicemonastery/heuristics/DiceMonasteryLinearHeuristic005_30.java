...
public class DiceMonasteryLinearHeuristic005_30 extends TunableParameters implements IStateHeuristic  {

    // From data gathered using 10s with Lin004 and a rollout of 100
    // AIC model with all data for 50+ visits
    // Regularisation at lambda = 9.7e-4
   ...
   //     retValue += state.monksIn(GATEHOUSE, playerId).stream().mapToDouble(Monk::getPiety).sum() * 0.00036;
        ...
        retValue += allStores.getOrDefault(BEER, 0) * 0.0098;
...
//        retValue += allStores.getOrDefault(VELLUM, 0) * -0.0119;
        ...
        retValue += state.getResource(playerId, SKEP, MEADOW) * -0.0047;
...
//        retValue += allStores.getOrDefault(VIVID_GREEN_INK, 0) * 0.0038;
        ...
        retValue += allMonks.stream().filter(m -> m.getPiety() == 2).count() * 0.0135;
...
//        retValue += allMonks.stream().mapToInt(Monk::getPiety).sum() * -0.00301;
        ...
        retValue += state.getVictoryPoints(playerId) * 0.0066;
        return retValue;

    }

    @Override
    protected DiceMonasteryLinearHeuristic005_30 _copy() {
        return this;
    }

    @Override
    protected boolean _equals(Object o) {
        return o instanceof DiceMonasteryLinearHeuristic005_30;
    }

    @Override
    public DiceMonasteryLinearHeuristic005_30 instantiate() {
        return this;
    }
