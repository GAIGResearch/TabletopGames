...
public class Connect4Heuristic extends TunableParameters implements IStateHeuristic {
    ...
    public double evaluateState(AbstractGameState gs, int playerId) {
        Connect4GameState ttgs = (Connect4GameState) gs;
        ...
        Token playerChar = Connect4Constants.playerMapping.get(playerId);
    ...
    private Pair<Integer, Integer> countColumns(Connect4GameState ttgs, int column, Token playerChar) {
    ...
    private Pair<Integer, Integer> countRows(Connect4GameState ttgs, int row, Token playerChar) {
        Pair<Integer, Integer> count = new Pair<>(0, 0);
        for (int x = 0; x < ttgs.gridBoard.getWidth(); x++) {
    ...
    private Pair<Integer, Integer> countPrimaryDiagonal(Connect4GameState ttgs, Token playerChar) {
        Pair<Integer, Integer> count = new Pair<>(0, 0);
        for (int x = 0; x < ttgs.gridBoard.getWidth(); x++) {
    ...
    private Pair<Integer, Integer> countSecondaryDiagonal(Connect4GameState ttgs, Token playerChar) {
        Pair<Integer, Integer> count = new Pair<>(0, 0);
        for (int x = 0; x < ttgs.gridBoard.getWidth(); x++) {
    ...
    protected Connect4Heuristic _copy() {
        Connect4Heuristic retValue = new Connect4Heuristic();
    ...
    protected boolean _equals(Object o) {
        if (o instanceof Connect4Heuristic) {
            Connect4Heuristic other = (Connect4Heuristic) o;
