...
public class SushiGoFeatureVector implements IActionFeatureVector {
    ...
    String[] names = new LinkedList<String>() {
        {
            ...
            // Pudding Features // TODO: if pudding, include it
            ...
            // Maki Features // TODO: if maki, include it
            ...
            // Tempura & Sashimi Features // TODO: if Tempura/Sashimi, include it
            ...
            // Dumpling Features // TODO: if Dumpling, include it
            for (String prefix : Arrays.asList("", "Next", "NextNext")) {
                for (String postfix : Arrays.asList("", "AndInHand", "AndInNextHand", "AndInHandAndInNextHand")) {
            ...
            // Wasabi Features // TODO: if wasabi, include it
            add("WasabiPlayed1");
            add("WasabiPlayed2+");
            ...
            // Chopstick Features // TODO: if chopstick, include it, remove card from hand.
            // TODO: if chopsticks used, ignore chopstick. remove random cards from next
            // hands
            ...
            // Hand Features // TODO: remove card from hand, remove random card from next
            // hand
    ...
    private String getSGCardName(SGCard card) {
        if (card.type == SGCardType.Maki)
            return card.type.name() + card.count;
    ...
    public double[] featureVector(AbstractAction action, AbstractGameState state, int playerID) {
        ...
        ChooseCard chooseCardAction = (ChooseCard) action;
        SGCard card = sggs.getPlayerHands().get(playerID).get(chooseCardAction.cardIdx);

        // Update hand and played lists with the chosen card
        sggs.getPlayerHands().get(playerID).remove(((ChooseCard) action).cardIdx);
        sggs.getPlayedCardTypes(card.type, playerID).increment(card.count);
        // Handle special cases
        if (Arrays.asList(SGCardType.SquidNigiri, SGCardType.SalmonNigiri, SGCardType.EggNigiri).contains(card.type))
            sggs.getPlayedCardTypes(SGCardType.Wasabi, playerID).decrement();
        if (chooseCardAction.useChopsticks)
