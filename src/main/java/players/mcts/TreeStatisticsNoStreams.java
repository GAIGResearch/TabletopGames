...
public class TreeStatisticsNoStreams {
    ...
    public TreeStatisticsNoStreams(SingleTreeNodeNoStreams root) {
        Queue<SingleTreeNodeNoStreams> nodeQueue = new ArrayDeque<>();
        nodeQueue.add(root);
        int greatestDepth = 0;
        int maxActions = 0;
        int totalActions = 0;
        while (!nodeQueue.isEmpty()) {
            SingleTreeNodeNoStreams node = nodeQueue.poll();
            if (node.depth < maxDepth) {
                ...
                for (SingleTreeNodeNoStreams child : node.children.values().stream()
