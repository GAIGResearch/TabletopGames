...
public class SingleTreeNodeNoStreams {
    // Root node of tree
    SingleTreeNodeNoStreams root;
    // Parent of this node
    SingleTreeNodeNoStreams parent;
    ...
    Map<AbstractAction, SingleTreeNodeNoStreams[]> children = new HashMap<>();
    // Depth of this node
    final int depth;
    ...
    public SingleTreeNodeNoStreams(MCTSPlayer player, SingleTreeNodeNoStreams parent, AbstractGameState state, Random rnd) {
        this.player = player;
        this.fmCallsCount = 0;
        this.parent = parent;
    ...
    public void mctsSearch(IStatisticLogger statsLogger) {
        ...
        while (!stop) {
            ...
            SingleTreeNodeNoStreams selected = treePolicy();
    ...
    private void logTreeStatistics(IStatisticLogger statsLogger, int numIters, long timeTaken) {
        ...
        TreeStatisticsNoStreams treeStats = new TreeStatisticsNoStreams(root);
...
/*        return children.values().stream()
                ...
                .toArray();*/
        int[] retValue = new int[state.getNPlayers()];
        for (SingleTreeNodeNoStreams[] nodes : children.values()) {
            if (nodes != null)
                for (int i = 0; i < nodes.length; i++) {
                    if (nodes[i] != null)
                        retValue[i] += nodes[i].nVisits;
                }
        }
        return retValue;
    }
     ...
     */
    private SingleTreeNodeNoStreams treePolicy() {

        SingleTreeNodeNoStreams cur = this;
...
*/
     ...
     */
    private SingleTreeNodeNoStreams expand() {
        ...
        SingleTreeNodeNoStreams tn = new SingleTreeNodeNoStreams(player, this, nextState, rnd);
        SingleTreeNodeNoStreams[] nodeArray = new SingleTreeNodeNoStreams[state.getNPlayers()];
     ...
     */
    private SingleTreeNodeNoStreams nextNodeInTree() {
        ...
        SingleTreeNodeNoStreams[] nodeArray = children.get(actionChosen);
        if (player.params.openLoop) {
            ...
            SingleTreeNodeNoStreams nextNode = nodeArray[nextPlayer];
            if (nextNode == null) {
                // need to create a new node
                nodeArray[nextPlayer] = new SingleTreeNodeNoStreams(player, this, state, rnd);
            ...
            Optional<SingleTreeNodeNoStreams> next = Arrays.stream(nodeArray).filter(Objects::nonNull).findFirst();
            if (next.isPresent()) {
                return next.get();
            } else {
    ...
    private AbstractAction ucb() {
        ...
        for (AbstractAction action : actionsFromState) {
            SingleTreeNodeNoStreams[] childArray = children.get(action);
            if (childArray == null)
    ...
    private void backUp(double[] result) {
        SingleTreeNodeNoStreams n = this;
        while (n != null) {
            n.nVisits++;
    ...
    public Map<AbstractAction, SingleTreeNodeNoStreams[]> getChildren() {
        return children;
    }
    ...
    public String toString() {
        ...
        retValue.append(new TreeStatisticsNoStreams(root).toString());
